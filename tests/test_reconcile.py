...
class TestComputeFluxParametersFromSpectrum:
    ...
    def test_complete_parameters(self):
        ...
        flux = np.where(
            e_centers < 1.0, 1e12 * np.exp(-e_centers / 0.025), 1e9 / e_centers
        )
...
class TestReconcileFluxParameters:
    ...
    def test_summary_generation(self):
        ...
        params = UnfoldedFluxParameters(
            f=20.0, f_uncertainty=2.0, alpha=0.05, alpha_uncertainty=0.02
        )
...
class TestQuickFCheck:
    ...
    def test_ok_within_tolerance(self):
        ...
        ok, computed_f, message = quick_f_check(
            flux, energy_bins, expected_f=0.58, tolerance=0.2
        )
    ...
    def test_not_ok_outside_tolerance(self):
        ...
        ok, computed_f, message = quick_f_check(
            flux, energy_bins, expected_f=5.0, tolerance=0.2
        )
