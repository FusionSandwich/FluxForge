...
class TestAutoCalibration:
    ...
    def test_auto_calibrate_synthetic(self):
        ...
        result = auto_calibrate(
            counts, isotopes=["Co-60"], initial_gain=3.0, min_peaks=2
        )
...
class TestGMAWorkflow:
    ...
    def test_experimental_database_json(self):
        ...
        from fluxforge.evaluation.gma_workflow import Experiment, ExperimentalDatabase
        import numpy as np
    ...
    def test_gls_workflow(self):
        ...
        post_error = sum(
            (p - t) ** 2 for p, t in zip(result.posterior_values, true_params)
        )
...
class TestStackedTarget:
    ...
    def test_stacked_target_energy_degradation(self):
        ...
        from fluxforge.physics.stacked_target import StackedTarget, Projectile

        stack = StackedTarget(projectile=Projectile.PROTON, beam_energy_MeV=20.0)
...
class TestAdvancedUnfolding:
    ...
    def test_adye_error_propagation(self):
        ...
        response = [
            [0.3 if abs(i - j) <= 1 else 0.1 for j in range(n_flux)]
            for i in range(n_meas)
        ]
        flux = [100, 150, 200, 150, 100]
        meas_cov = [
            [100 if i == j else 0 for j in range(n_meas)] for i in range(n_meas)
        ]
    ...
    def test_mlem_with_covariance(self):
        ...
        response = [
            [0.3 * math.exp(-0.5 * (g - 1.5 * i) ** 2) for g in range(n_flux)]
            for i in range(n_meas)
        ]
...
class TestLogSmoothness:
    ...
    def test_mlem_with_smoothness(self):
        ...
        assert log_smoothness_penalty(result_smooth.flux) <= log_smoothness_penalty(
            result_no_smooth.flux
        )
