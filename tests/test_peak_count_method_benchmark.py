...
from fluxforge.examples.rafm_workflow import default_paths, qg_reference_peaks, workflow_profile_energy_calibration
from fluxforge.io.flux_wire import read_processed_txt, read_raw_asc
from examples.RAFM_irradiation.compare_peak_count_methods import compute_current_method_results
...
def test_qg_benchmark_predictions_match_reference_for_representative_ti_sample():
    ...
    config = json.loads((paths.metadata_root / "workflow_config.json").read_text(encoding="utf-8"))
    ...
    reference_data = read_processed_txt(qg_path, profile_name=str(config["profile_name"]))
    ...
    predictions = compute_current_method_results(raw_data, reference_data, background_data, config)
