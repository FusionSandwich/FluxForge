...
"""
    ...
    def test_read_with_optional_columns(self, tmp_path):
        csv_file = tmp_path / "rates.csv"
        csv_file.write_text(
            """reaction,rate,uncertainty,half_life_s,target_atoms,notes
...
class TestSTAYSLBundleIO:
    ...
    def test_export_import_roundtrip(self, tmp_path):
        ...
        np.testing.assert_array_almost_equal(
            loaded.energy_bounds_eV, original.energy_bounds_eV
        )
        assert loaded.reaction_ids == original.reaction_ids
        np.testing.assert_array_almost_equal(
            loaded.measured_rates, original.measured_rates, decimal=5
        )
        np.testing.assert_array_almost_equal(
            loaded.response_matrix, original.response_matrix, decimal=5
        )
