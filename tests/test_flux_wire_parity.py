...
def test_raw_flux_wire_parity():
    ...
    out_of_tolerance = [c for c in comparisons if abs(c.diff_pct) > 1.0]
    assert not out_of_tolerance, "Raw/processed parity exceeded 1% for: " + ", ".join(
        f"{c.sample_id}:{c.isotope}({c.diff_pct:+.2f}%)" for c in out_of_tolerance
    )


@pytest.mark.parametrize(
    "profile_name", ["astm_inl_dosimetry", "us_astm_reactor_dosimetry"]
)
def test_astm_profile_aliases_preserve_flux_wire_qg_parity(profile_name: str):
    ...
    canonical_background = read_raw_asc(
        BACKGROUND_ASC, profile_name="rafm_25cm"
    ).spectrum
    ...
    for isotope in ("Sc46",):
        assert alias_analysis.nuclide_activities[isotope][
            "activity_bq"
        ] == pytest.approx(
...
def test_flux_wire_analysis_does_not_depend_on_processed_targets():
    ...
    assert sorted(
        (peak.isotope, round(peak.energy_keV, 3), round(peak.net_counts, 3))
        for peak in with_ref.peaks
    ) == sorted(
        (peak.isotope, round(peak.energy_keV, 3), round(peak.net_counts, 3))
        for peak in without_ref.peaks
    )
