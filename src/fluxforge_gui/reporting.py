...
def summarize_gui_unfold_result(payload: dict[str, Any]) -> str:
    ...
    if flux_uncertainty.size == flux.size and flux.size:
        valid = flux > 0.0
        if np.any(valid):
            rel_unc = np.median(flux_uncertainty[valid] / np.maximum(flux[valid], 1e-30))
...
def summarize_gui_activity_result(payload: dict[str, Any]) -> str:
    ...
    activities = np.asarray([float(item.get("activity_Bq", 0.0) or 0.0) for item in lines], dtype=float)
    uncertainties = np.asarray([float(item.get("activity_unc_Bq", 0.0) or 0.0) for item in lines], dtype=float)
    rel_unc = np.divide(uncertainties, np.maximum(np.abs(activities), 1e-30), out=np.zeros_like(uncertainties), where=np.abs(activities) > 0.0)
    radioactive_mass = np.asarray([float(item.get("radioactive_mass_g", 0.0) or 0.0) for item in lines], dtype=float)
    specific_activity = np.asarray([float(item.get("specific_activity_Bq_g", 0.0) or 0.0) for item in lines], dtype=float)
    ...
    if np.any(specific_activity > 0.0):
        parts.append(f"Max specific activity: {float(np.max(specific_activity)):.4g} Bq/g")
    return " | ".join(parts)
...
def summarize_gui_rate_result(payload: dict[str, Any]) -> str:
    ...
    rate_values = np.asarray([float(item.get("rate", 0.0) or 0.0) for item in rates], dtype=float)
    uncertainties = np.asarray([float(item.get("uncertainty", 0.0) or 0.0) for item in rates], dtype=float)
    rel_unc = np.divide(uncertainties, np.maximum(np.abs(rate_values), 1e-30), out=np.zeros_like(uncertainties), where=np.abs(rate_values) > 0.0)
...
def summarize_gui_validation_result(payload: dict[str, Any]) -> str:
    ...
    if truth_flux.size and predicted_flux.size:
        ...
        parts.append(f"Flux norm ratio: {float(np.sum(np.abs(predicted_flux))) / max(truth_norm, 1e-30):.4g}")
    if residuals.size:
...
def build_gui_k0_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
    ...
    if standard:
        ...
        lines.append("-------")
        lines.append(f"standard_k0_library: {standard.get('library_id', 'unknown')} @ {standard.get('version', 'unknown')}")
    return "\n".join(lines) + "\n"


def build_gui_astm_e2005_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
    ...
    for ft in payload.get("fluence_transfers", []):
        ...
        lines.append(f"  Fluence: {ft.get('fluence_rate_cm2_s')} +- {ft.get('fluence_rate_unc_cm2_s')}")
    for si in payload.get("spectral_indices", []):
        lines.append(f"Index: {si.get('index_id')}")
        lines.append(f"  Measured: {si.get('measured_index')} +- {si.get('measured_index_unc')}")
        lines.append(f"  Calculated: {si.get('calculated_index')} +- {si.get('calculated_index_unc')}")
        lines.append(f"  C/E Ratio: {si.get('c_e_ratio')} +- {si.get('c_e_ratio_unc')}")
    return "\n".join(lines) + "\n"


def build_gui_astm_e261_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
...
def build_gui_astm_e262_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
...
def build_gui_astm_e3376_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
...
def discover_gui_validation_report_inputs(results_root: str | Path) -> dict[str, str]:
    ...
    return {
        ...
        "unfold_file": "" if discovered.get("preferred_unfold_file") is None else str(discovered["preferred_unfold_file"]),
        "validation_file": "",
...
def render_gui_activity_result(
    ...
    *,
    ...
    if not lines:
        ...
        ax.text(0.5, 0.5, "No line activities loaded.", ha="center", va="center", transform=ax.transAxes)
        fig.tight_layout()
        return fig
    ...
    uncertainties = np.asarray([float(item.get("activity_unc_Bq", 0.0) or 0.0) for item in lines], dtype=float)
    radioactive_mass = np.asarray([float(item.get("radioactive_mass_g", 0.0) or 0.0) for item in lines], dtype=float)
    labels = [f"{item.get('isotope', 'line')}\n{float(item.get('energy_keV', 0.0) or 0.0):.1f} keV" for item in lines]
    x = np.arange(len(lines), dtype=float)
    ax.errorbar(x, activities, yerr=uncertainties if uncertainties.size == activities.size else None, fmt="o", color="#1f77b4", ecolor="#9ecae1", elinewidth=1.1, capsize=3, label="Activity")
    ...
    if np.any(radioactive_mass > 0.0):
        mass_ax = ax.twinx()
        mass_ax.plot(x, radioactive_mass, color="#d62728", marker="s", linewidth=1.0, label="Radioactive mass")
...
def render_gui_rate_result(
    ...
    *,
    ...
    if not rates:
        ...
        ax.text(0.5, 0.5, "No reaction rates loaded.", ha="center", va="center", transform=ax.transAxes)
        fig.tight_layout()
        return fig

    values = np.asarray([float(item.get("rate", 0.0) or 0.0) for item in rates], dtype=float)
    uncertainties = np.asarray([float(item.get("uncertainty", 0.0) or 0.0) for item in rates], dtype=float)
    labels = [str(item.get("reaction_id", f"R{i + 1}")) for i, item in enumerate(rates)]
    x = np.arange(len(rates), dtype=float)
    ax.errorbar(x, values, yerr=uncertainties if uncertainties.size == values.size else None, fmt="o", color="#2ca02c", ecolor="#98df8a", elinewidth=1.1, capsize=3)
...
def render_gui_validation_result(
    ...
    *,
    ...
    if truth_flux.size == 0 or predicted_flux.size == 0:
        ...
        top_ax.text(0.5, 0.5, "No validation spectra loaded.", ha="center", va="center", transform=top_ax.transAxes)
        bottom_ax.axis("off")
        fig.tight_layout()
        return fig

    x = np.arange(1, max(truth_flux.size, predicted_flux.size) + 1, dtype=float)
    top_ax.step(x[: truth_flux.size], truth_flux, where="mid", color="#1f77b4", linewidth=1.6, label="Truth")
    top_ax.step(x[: predicted_flux.size], predicted_flux, where="mid", color="#d62728", linewidth=1.4, label="Predicted")
    ...
    if residuals.size:
        bottom_ax.bar(x[: residuals.size], residuals, color="#ff7f0e", alpha=0.8, edgecolor="black")
...
def render_gui_unfold_result(
    ...
    *,
    ...
    measured_rate_unc = np.asarray(diagnostics.get("measured_rate_uncertainties", []), dtype=float)
    predicted_rate_unc = np.asarray(diagnostics.get("predicted_rate_uncertainties", []), dtype=float)
    residuals = np.asarray(diagnostics.get("rate_residuals", []), dtype=float)
    pulls = np.asarray(diagnostics.get("rate_pulls", []), dtype=float)
    reaction_labels = list(diagnostics.get("reactions", []) or payload.get("reactions", []) or [])

    has_rate_panel = measured_rates.size and predicted_rates.size and measured_rates.shape == predicted_rates.shape
    has_covariance = covariance.ndim == 2 and covariance.shape[0] == flux.size and covariance.shape[1] == flux.size and np.any(np.abs(covariance) > 0.0)
    has_residual_panel = residuals.size and pulls.size and residuals.shape == pulls.shape
    needs_grid = has_rate_panel or has_covariance or chi2_history.size or has_residual_panel
    ...
    if flux.size == 0:
        ...
        flux_ax.text(0.5, 0.5, "No unfolded flux loaded.", ha="center", va="center", transform=flux_ax.transAxes)
        fig.tight_layout()
        return fig

    if boundaries.size == flux.size + 1:
        x_values = boundaries[:-1]
        flux_ax.step(x_values, flux, where="post", color="#1f77b4", linewidth=1.8, label="Flux")
        if np.all(boundaries > 0):
            flux_ax.set_xscale("log")
            flux_ax.set_xlim(float(boundaries[0]), float(boundaries[-1]))
        centers = np.sqrt(boundaries[:-1] * boundaries[1:]) if np.all(boundaries[:-1] > 0) and np.all(boundaries[1:] > 0) else 0.5 * (boundaries[:-1] + boundaries[1:])
    else:
        centers = np.arange(1, flux.size + 1, dtype=float)
        flux_ax.plot(centers, flux, color="#1f77b4", linewidth=1.8, label="Flux")

    if flux_uncertainty.size != flux.size and covariance.ndim == 2 and covariance.shape[0] == flux.size and covariance.shape[1] == flux.size:
    ...
    if flux_uncertainty.size == flux.size and np.any(flux_uncertainty > 0.0):
        flux_ax.errorbar(centers, flux, yerr=flux_uncertainty, fmt="none", ecolor="#6baed6", elinewidth=1.0, alpha=0.9, label="1σ")

    if np.all(flux > 0):
        flux_ax.set_yscale("log")
    flux_ax.set_title(f"Unfolded spectrum ({str(payload.get('method') or 'unknown').upper()})")
    flux_ax.set_xlabel("Energy (eV)")
    flux_ax.set_ylabel("Flux (a.u.)")
    ...
    if parity_ax is not None:
        if has_rate_panel:
            min_val = float(max(min(np.min(measured_rates), np.min(predicted_rates)), 1e-30))
            max_val = float(max(np.max(measured_rates), np.max(predicted_rates), min_val * 10.0))
            parity_ax.plot([min_val, max_val], [min_val, max_val], "k--", linewidth=1.0, label="1:1")
            parity_ax.errorbar(
                measured_rates,
                predicted_rates,
                xerr=measured_rate_unc if measured_rate_unc.size == measured_rates.size else None,
                yerr=predicted_rate_unc if predicted_rate_unc.size == predicted_rates.size else None,
                fmt="o",
                color="#2ca02c",
                ecolor="#98df8a",
                elinewidth=1.0,
            ...
            for index, label in enumerate(reaction_labels[: measured_rates.size]):
                parity_ax.annotate(label, (measured_rates[index], predicted_rates[index]), textcoords="offset points", xytext=(4, 4), fontsize=7)
            ...
            parity_ax.plot(np.arange(1, chi2_history.size + 1, dtype=float), chi2_history, color="#d62728", linewidth=1.6)
    ...
    if residual_ax is not None:
        if has_residual_panel:
            ...
            labels = reaction_labels[: residuals.size] if reaction_labels else [f"R{i+1}" for i in range(residuals.size)]
            ...
            pull_ax.axhline(2.0, color="#9467bd", linestyle="--", linewidth=0.9, alpha=0.6)
            pull_ax.axhline(-2.0, color="#9467bd", linestyle="--", linewidth=0.9, alpha=0.6)
    ...
    if diagnostics_ax is not None:
        if has_covariance:
            ...
            image = diagnostics_ax.imshow(corr, cmap="RdBu_r", vmin=-1.0, vmax=1.0, aspect="auto")
            ...
            diagnostics_ax.plot(np.arange(1, chi2_history.size + 1, dtype=float), chi2_history, color="#d62728", linewidth=1.6)
...
def build_gui_rafm_validation_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
    """Return the text shown in the GUI RAFM validation preview pane."""

    lines = ["FluxForge RAFM Validation Preview", "================================", ""]
    lines.append(f"Bundle: {Path(bundle_path)}")
    ...
    lines.append(f"QG consistency flags: {payload.get('qg_internal_consistency_flags', 0)}")
    lines.append(f"FluxForge line consistency flags: {payload.get('fluxforge_line_consistency_flags', 0)}")
...
def build_gui_rafm_qg_benchmark_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
    """Return the text shown in the GUI RAFM QG benchmark preview pane."""

    lines = ["FluxForge RAFM QG Benchmark Preview", "==================================", ""]
    lines.append(f"Bundle: {Path(bundle_path)}")
    lines.append(f"Overall pass: {'yes' if payload.get('overall_passed', True) else 'no'}")
...
def build_gui_rafm_branch_comparison_preview(payload: dict[str, Any], bundle_path: str | Path) -> str:
    ...
    lines.append(f"Median |activity rel err|: {float(payload.get('median_abs_activity_rel_error', 0.0) or 0.0):.4g}")
    lines.append(f"Max |activity rel err|: {float(payload.get('max_abs_activity_rel_error', 0.0) or 0.0):.4g}")
    lines.append(f"Median |rate rel err|: {float(payload.get('median_abs_rate_rel_error', 0.0) or 0.0):.4g}")
    lines.append(f"Max |rate rel err|: {float(payload.get('max_abs_rate_rel_error', 0.0) or 0.0):.4g}")
    lines.append("")
