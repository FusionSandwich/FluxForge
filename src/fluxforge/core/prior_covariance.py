...
class PriorCovarianceModel(Enum):
    ...
    FRACTIONAL_REGIONAL = (
        "fractional_regional"  # Different uncertainty by energy region
    )
    LETHARGY_CORRELATED = (
        "lethargy_correlated"  # Correlation decays with lethargy distance
    )
...
def summarize_prior_uncertainty_by_region(
    ...
    for region in regions:
        ...
        mask = (group_energies >= region.energy_low_ev) & (
            group_energies < region.energy_high_ev
        )
