...
def rebin_spectrum(
    spectrum: SpectrumData, new_edges: NDArray, method: str = "interpolate"
) -> SpectrumData:
    """
...
def listmode_to_histogram(
    events: NDArray, edges: NDArray, weights: Optional[NDArray] = None
) -> SpectrumData:
    """
    ...
    return SpectrumData(counts=counts, edges=edges)
...
class Calibration:
    ...
    def __init__(
        ...
        self.degree = degree
        self.coefficients = (
            coefficients if coefficients is not None else np.array([0.0, 1.0])
        )
    ...
    def inverse(
        self, y: Union[float, NDArray], x_guess: Optional[float] = None
    ) -> Union[float, NDArray]:
        """
