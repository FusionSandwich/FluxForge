...
class EfficiencyCurve:
    ...
    def efficiency_uncertainty(
        ...
        if model_type == "constant":
            ...
            cal_uncertainties = np.array(
                self.uncertainty_model.get("uncertainties", [])
            )
...
def distance_correction(
    d1: float, d2: float, detector_radius: Optional[float] = None
) -> float:
    """
    ...
    """
    if detector_radius is not None and (
        d1 < 3 * detector_radius or d2 < 3 * detector_radius
    ):
