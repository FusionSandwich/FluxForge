...
FLUXFORGE_GAMMA_DATA = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "gamma_lines.json"
)
...
class DecayData:
    ...
    def strongest_gamma_lines(
        self, n: int = 5, min_intensity: float = 0.0
    ) -> List[GammaLine]:
...
class GammaDatabase:
    ...
    def _parse_nuclide(self, name: str, data: dict) -> DecayData:
        ...
        decay = DecayData(
            nuclide=name, zai=data.get("zai", 0), halflife=data.get("halflife", 0.0)
        )
    ...
    def get(
        self, nuclide: str, default: Optional[DecayData] = None
    ) -> Optional[DecayData]:
    ...
    def nuclides_with_gamma(self) -> List[str]:
        ...
        return [
            name for name, decay in self._nuclides.items() if len(decay.gamma_lines) > 0
        ]
    ...
    def get_calibration_sources(self) -> Dict[str, DecayData]:
        ...
        return {
            name: self._nuclides[name] for name in calibration if name in self._nuclides
        }
        ...
        for decay in self._nuclides.values():
            ...
            if energy_range_keV:
                ...
                if len(energies) == 0:
                    continue
                if not any(
                    energy_range_keV[0] <= e <= energy_range_keV[1] for e in energies
                ):
                    continue
    ...
    def summary(self) -> str:
        ...
        return (
            f"GammaDatabase: {self.n_nuclides} nuclides, "
...
def find_gamma_matches(
    energy_keV: float, tolerance_keV: float = 1.0
) -> List[Tuple[str, GammaLine]]:
    """
...
def identify_nuclides(
    peak_energies_keV: List[float], tolerance_keV: float = 1.0
) -> Dict[str, list]:
    """
...
def parse_nuclide_name(name: str) -> Tuple[str, int, int]:
    ...
    """
    return parse_gamma_nuclide_name(name)
...
def zai_to_name(zai: int) -> str:
    ...
    return format_isotope_name(element, a, i, separator="")
