...
HALF_LIVES_S: Dict[str, float] = {
    ...
    "Sc-47": 289353.6,  # 3.349 days
    "Sc-48": 157212.0,  # 43.67 h
    ...
    "Ti-46": float("inf"),  # Stable
    "Ti-47": float("inf"),  # Stable
    "Ti-48": float("inf"),  # Stable
    "Cu-63": float("inf"),  # Stable
    "Fe-58": float("inf"),  # Stable
    "In-113": float("inf"),  # Stable
    "In-115": float("inf"),  # Stable
...
def parse_isotope(name: str) -> Tuple[str, int, int]:
    ...
    """
    return parse_nndc_isotope_name(name)


def format_isotope(element: str, A: int, m: int = 0) -> str:
    """Format isotope as 'El-A' or 'El-Am'."""
    return format_isotope_name(element, A, m, separator="-")
...
class IsotopeQuantity:
    ...
    def time_when(self, activity_Bq: float) -> float:
        ...
        if lam == 0:
            return (
                float("inf") if self.activity_Bq >= activity_Bq else self.reference_time
            )
...
def get_nuclear_data(
    isotope: str, include_gammas: bool = True, include_decay: bool = True
) -> Dict[str, Any]:
    """
...
def list_isotopes_with_gammas(
    e_min: float = 0, e_max: float = 3000, i_min: float = 0.0
) -> List[Dict[str, Any]]:
    """
...
if __name__ == "__main__":
    ...
    for g in gammas_500_1500[:5]:
        print(
            f"  {g['isotope']}: {g['energy_keV']:.1f} keV ({g['intensity']*100:.1f}%)"
        )

    print("\n[OK] nndc module tests passed!")
