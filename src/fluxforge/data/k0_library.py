...
class K0LibraryRecord:
	...
	source_note: str = "FluxForge bundled starter library derived from existing add_gui seed values"
...
_K0_STARTER_RECORDS: tuple[K0LibraryRecord, ...] = (
	K0LibraryRecord("Au-198", "Au-197", "Au", 411.8, 0.9558, 2.6944 * 24 * 3600, 1.0, 0.0, 15.71, 1.5, 5.65, 98.65, 1550.0, 1.0, 196.967),
	K0LibraryRecord("Co-60", "Co-59", "Co", 1332.5, 0.9998, 5.2714 * 365.25 * 24 * 3600, 1.320, 0.9, 1.99, 2.0, 132.0, 37.18, 74.0, 1.0, 58.933, additional_gammas=((1173.2, 0.9985),)),
	K0LibraryRecord("Sc-46", "Sc-45", "Sc", 889.3, 0.99984, 83.79 * 24 * 3600, 8.79e-3, 1.3, 0.435, 4.0, 4.5, 27.5, 12.0, 1.0, 44.956, additional_gammas=((1120.5, 0.99987),)),
	K0LibraryRecord("Fe-59", "Fe-58", "Fe", 1099.2, 0.565, 44.50 * 24 * 3600, 5.34e-5, 2.0, 0.91, 8.0, 230.0, 1.31, 1.19, 0.00282, 55.845, additional_gammas=((1291.6, 0.432),)),
	K0LibraryRecord("Cu-64", "Cu-63", "Cu", 1345.8, 0.00473, 12.701 * 3600, 3.88e-3, 2.5, 1.11, 4.0, 580.0, 4.5, 5.0, 0.6917, 63.546),
	K0LibraryRecord("In-114m", "In-113", "In", 190.3, 0.1556, 49.51 * 24 * 3600, 3.16e-2, 2.0, 31.5, 3.0, 1.45, 4.0, 126.0, 0.0429, 114.818),
	K0LibraryRecord("Mn-56", "Mn-55", "Mn", 846.8, 0.9887, 2.5789 * 3600, 4.88e-3, 1.2, 0.67, 3.5, 337.0, 13.3, 8.9, 1.0, 54.938, additional_gammas=((1810.7, 0.272), (2113.1, 0.143))),
	K0LibraryRecord("Na-24", "Na-23", "Na", 1368.6, 0.9999, 14.997 * 3600, 4.68e-2, 1.3, 0.59, 4.0, 2850.0, 0.530, 0.31, 1.0, 22.990, additional_gammas=((2754.0, 0.9994),)),
	K0LibraryRecord("Cr-51", "Cr-50", "Cr", 320.1, 0.0991, 27.701 * 24 * 3600, 8.41e-4, 2.0, 2.22, 6.0, 1820.0, 15.8, 35.0, 0.04345, 51.996),
	K0LibraryRecord("Zn-65", "Zn-64", "Zn", 1115.5, 0.506, 243.66 * 24 * 3600, 3.05e-4, 2.0, 1.91, 5.0, 2560.0, 0.76, 1.45, 0.4863, 65.38),
	K0LibraryRecord("As-76", "As-75", "As", 559.1, 0.451, 26.32 * 3600, 1.42e-2, 2.5, 2.50, 6.0, 1360.0, 4.30, 10.8, 1.0, 74.922),
	K0LibraryRecord("W-187", "W-186", "W", 685.8, 0.273, 23.72 * 3600, 1.05e-4, 3.0, 19.3, 7.0, 18.8, 39.5, 762.0, 0.2843, 183.84),
)
...
BUILTIN_AUXILIARY_CORRECTION_LIBRARY = GovernedLibrary(
	...
	records={
		...
		for record in (
			AuxiliaryCorrectionRecord("54Fe(n,p)54Mn", "threshold", "fast_flux_monitor", "IRDFF-II", "Common fast-flux monitor pathway for threshold-interference bookkeeping."),
			AuxiliaryCorrectionRecord("58Ni(n,p)58Co", "threshold", "fast_flux_monitor", "IRDFF-II", "Optional fast-flux monitor pathway kept separate from standard k0 constants."),
			AuxiliaryCorrectionRecord("27Al(n,p)27Mg", "threshold", "fast_flux_monitor", "IRDFF-II", "Short-lived fast-flux support pathway; numerical corrections are workflow-specific."),
		)
	},
)
...
def governed_library_from_dict(payload: Dict[str, Any], *, library_kind: str = "standard") -> GovernedLibrary:
	...
	if isinstance(record_items, list):
		...
		for item in record_items:
			if isinstance(item, dict):
				key = str(item.get("product_isotope") or item.get("reaction_id") or f"record_{len(record_iterable) + 1}")
	...
	return GovernedLibrary(
		library_id=str(payload.get("library_id") or f"fluxforge.k0.{library_kind}.external"),
		...
		provenance_note=str(payload.get("provenance_note") or "Loaded from external file."),
		records=coerced_records,
	)


def load_governed_library(path: str | Path, *, library_kind: str = "standard") -> GovernedLibrary:
...
def set_active_auxiliary_correction_library(library: GovernedLibrary | None = None) -> GovernedLibrary:
	"""Set the active auxiliary correction library and return it."""

	global _ACTIVE_AUXILIARY_CORRECTION_LIBRARY
	_ACTIVE_AUXILIARY_CORRECTION_LIBRARY = library or BUILTIN_AUXILIARY_CORRECTION_LIBRARY
