...
class AttenuationData:
    ...
    def get_mu_rho(self, energy_keV: Union[float, np.ndarray]) -> np.ndarray:
        ...
        interp = interpolate.interp1d(
            log_E, log_mu, kind="linear", bounds_error=False, fill_value="extrapolate"
        )
...
def calculate_transmission(
    material: str, energy_keV: Union[float, np.ndarray], thickness_cm: float
) -> np.ndarray:
    """
...
def mixture_mu_rho(
    composition: Dict[str, float], energy_keV: Union[float, np.ndarray]
) -> np.ndarray:
    """
