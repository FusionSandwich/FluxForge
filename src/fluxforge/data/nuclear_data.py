    ...
    def get_at_temperature(
        ...
        for i in range(len(temps) - 1):
            if temps[i] <= temperature_K <= temps[i + 1]:
                ...
                f = (np.sqrt(temperature_K) - np.sqrt(t_lo)) / (
                    np.sqrt(t_hi) - np.sqrt(t_lo)
                )
    ...
    for T in temperatures:
        if T == base_data.temperature_K:
            ...
            result[T] = TemperatureData(
                ...
                uncertainties=(
                    base_data.uncertainties.copy()
                    if base_data.uncertainties is not None
                    else None
                ),
            )
