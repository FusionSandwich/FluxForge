...
def condition_covariance_svd(
    ...
    diagnostics = {
        "original_condition": (
            s_original[0] / s_original[-1] if s_original[-1] > 0 else np.inf
        ),
...
def read_endf_mf33_section(
    ...
    if section.list_records:
        flat = (
            np.concatenate(section.list_records)
            if len(section.list_records) > 1
            else section.list_records[0]
        )
            ...
            "reactions": [
                {
                    ...
                    "covariance_degraded_warnings": list(
                        self._degraded_warnings.get((za, mt), [])
                    ),
                }
...
def create_default_dosimetry_library() -> CovarianceLibrary:
    ...
    energy_boundaries = np.logspace(
        np.log10(1e-5), np.log10(20e6), 26  # 10 meV  # 20 MeV
    )
