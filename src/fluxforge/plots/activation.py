...
class ComparisonResult:
    ...
    def ratio_CE_uncertainty(self) -> float:
        ...
        if self.experimental_Bq > 0 and self.simulated_Bq > 0:
            ...
            rel_sim = (
                self.simulated_unc_Bq / self.simulated_Bq
                if self.simulated_Bq > 0
                else 0
            )
    ...
    def difference_percent(self) -> float:
        ...
        if self.experimental_Bq > 0:
            return (
                (self.simulated_Bq - self.experimental_Bq) / self.experimental_Bq * 100
            )
        return np.nan
...
def plot_activity_comparison_bar(
    ...
    bars1 = ax1.bar(
        ...
        capsize=3,
        label="Experimental",
        color=SOURCE_COLORS["experimental"],
        alpha=0.8,
        edgecolor="black",
        linewidth=0.5,
    )
...
def plot_ce_ratio_summary(
    ...
    # Reference lines
    ax.axhline(
        y=1.0, color="black", linestyle="-", linewidth=1.5, label="Perfect agreement"
    )
    ax.axhline(y=0.8, color="gray", linestyle="--", alpha=0.5)
    ax.axhline(y=1.2, color="gray", linestyle="--", alpha=0.5)
    ax.fill_between(
        [-0.5, len(comparisons) - 0.5],
        0.8,
        1.2,
        color="green",
        alpha=0.1,
...
def plot_cd_ratio_analysis(
    ...
    colors = [
        "#2ca02c" if r > 2 else "#ff7f0e" if r > 1.5 else "#d62728" for r in ratios
    ]
