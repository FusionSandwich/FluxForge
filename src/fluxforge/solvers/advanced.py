...
def levenberg_marquardt(
    ...
    message = (
        "Converged" if converged else f"Max iterations ({config.max_iter}) reached"
    )
...
def estimate_unknown_uncertainty(
    residuals: np.ndarray, covariance: np.ndarray, method: str = "ml"
) -> float:
    """
