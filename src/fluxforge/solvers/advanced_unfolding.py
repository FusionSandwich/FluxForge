"""
Advanced Unfolding Module - Epics W (PyUnfold) and Y (benchmark)
...
def adye_error_propagation(
    ...
    if method == "bayes":
        ...
        for g1 in range(n_flux):
            for g2 in range(n_flux):
                RTR[g1][g2] = sum(
                    response[i][g1] * response[i][g2] for i in range(n_meas)
                )
        ...
        for g in range(n_flux):
            for j in range(n_meas):
                RT_V[g][j] = sum(
                    response[i][g] * measurement_cov[i][j] for i in range(n_meas)
                )
        ...
        for g1 in range(n_flux):
            for g2 in range(n_flux):
                RT_V_R[g1][g2] = sum(
                    RT_V[g1][j] * response[j][g2] for j in range(n_meas)
                )
        ...
        for g1 in range(n_flux):
            for g2 in range(n_flux):
                temp[g1][g2] = sum(
                    RTR_inv[g1][k] * RT_V_R[k][g2] for k in range(n_flux)
                )
...
def compute_ddJ_convergence(
    J_history: List[float], min_length: int = 10
) -> Tuple[float, float]:
    """
...
def mlem_with_covariance(
    ...
    for it in range(1, max_iters + 1):
        ...
        if verbose and it % 100 == 0:
            print(
                f"  MLEM-cov iter {it}: chi2/dof = {chi2_per_dof:.4f}, ddJ = {ddJ_history[-1]:.2e}"
            )
        ...
        if convergence_mode == "ddJ" and len(ddJ_history) >= 10:
            if ddJ_history[-1] < tolerance:
                ...
                if verbose:
                    print(
                        f"  MLEM-cov converged at iter {it}: ddJ = {ddJ_history[-1]:.2e}"
                    )
                break
            ...
            if verbose:
                print(
                    f"  MLEM-cov converged at iter {it}: rel_change = {max_rel_change:.2e}"
                )
            break
            ...
            if verbose:
                print(
                    f"  MLEM-cov converged at iter {it}: chi2/dof = {chi2_per_dof:.4f}"
                )
            break
...
if __name__ == "__main__":
    ...
    if result.flux_uncertainty:
        rel_unc = [
            u / f if f > 0 else 0 for u, f in zip(result.flux_uncertainty, result.flux)
        ]
    ...
    print(
        f"  Converged: {result_smooth.converged} after {result_smooth.iterations} iterations"
    )
