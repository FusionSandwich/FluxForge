...
def chi2_test_statistic(
    phi_curr: np.ndarray, phi_prev: np.ndarray, sigma: Optional[np.ndarray] = None
) -> float:
    """
...
def bayes_factor_test_statistic(
    phi_curr: np.ndarray,
    phi_prev: np.ndarray,
...
def compute_test_statistic(
    ...
    **kwargs
    ...
    if ts_type == TestStatisticType.KS:
        ...
        return bayes_factor_test_statistic(
            phi_curr, phi_prev, kwargs.get("data"), kwargs.get("response")
        )
...
def check_convergence(
    ...
    **kwargs
    ...
    return ConvergenceResult(
        statistic=stat, converged=converged, ts_type=ts_type, threshold=threshold
    )
...
def power_law_prior(
    n_bins: int, e_min: float = 1e-3, e_max: float = 20.0, index: float = -1.0
) -> np.ndarray:
    """
...
def spline_regularize(
    phi: np.ndarray, smoothing: float = 0.1, order: int = 3
) -> np.ndarray:
    """
...
def grouped_spline_regularize(
    phi: np.ndarray, groups: list, smoothing: float = 0.1
) -> np.ndarray:
    """
...
def iterative_bayesian_unfold(
    ...
    history = {"flux": [phi.copy()], "test_stat": [], "converged": False, "n_iter": 0}
    ...
    for iteration in range(max_iter):
        ...
        conv = check_convergence(
            phi_new, phi, ts_threshold, ts_type, data=data, response=response
        )
...
if __name__ == "__main__":
    ...
    R = np.array([[0.8, 0.1, 0.1], [0.1, 0.8, 0.1], [0.1, 0.1, 0.8]])
