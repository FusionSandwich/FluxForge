...
from fluxforge.k0_physics import sdc_factor as _shared_sdc_factor
...
def list_non_1v_isotopes() -> List[str]:
    ...
    return [
        iso
        for iso, data in WESTCOTT_DATA.items()
...
def calculate_sdc_factors(
    ...
    tuple
        (S, D, C) factors
    """
    S, D, C, _ = _shared_sdc_factor(half_life_s, t_irr_s, t_decay_s, t_count_s)
    return S, D, C
...
class TRIGAk0Workflow:
    ...
    def characterize_flux_cd_ratio(
        ...
        for element in bare_activities:
            if element in cd_activities and element in STANDARD_MONITORS:
                unc_bare = (
                    uncertainties_bare.get(element, 0.05)
                    if uncertainties_bare
                    else 0.05
                )
                unc_cd = (
                    uncertainties_cd.get(element, 0.07) if uncertainties_cd else 0.07
                )
    ...
    def calculate_concentration(
        ...
        denominator = comparator_peak_area / (
            tm * S_c * D_c * C_c * comparator_mass_ug * 1e-6
        )
