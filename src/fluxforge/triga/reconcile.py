...
def compute_flux_parameters_from_spectrum(
    ...
    alpha, alpha_unc = compute_alpha_from_spectrum(flux, energy_bins, flux_uncertainty)
    ...
    phi_fast = (
        np.sum(flux[fast_mask] * delta_e[fast_mask]) if np.any(fast_mask) else 0.0
    )
...
def reconcile_flux_parameters(
    ...
    result.f_sigma = (
        abs(result.f_difference) / combined_f_unc if combined_f_unc > 0 else 0
    )

    result.alpha_difference = unfolded_params.alpha - cdratio_alpha
    combined_alpha_unc = np.sqrt(
        unfolded_params.alpha_uncertainty**2 + cdratio_alpha_unc**2
    )
    result.alpha_sigma = (
        abs(result.alpha_difference) / combined_alpha_unc
        if combined_alpha_unc > 0
        else 0
    )
    ...
    result.diagnostics = {
        ...
        "f_relative_difference": (
            result.f_difference / cdratio_f if cdratio_f > 0 else 0
        ),
        "alpha_relative_difference": (
            result.alpha_difference / cdratio_alpha if cdratio_alpha != 0 else 0
        ),
...
def reconcile_with_unfold_result(
    ...
    if hasattr(unfold_result, "covariance") and unfold_result.covariance is not None:
        ...
        hasattr(unfold_result, "uncertainty") and unfold_result.uncertainty is not None
    ):
    ...
    if energy_bins_ev is None:
        if hasattr(unfold_result, "energy_bins"):
            ...
            raise ValueError(
                "Energy bins must be provided or available in unfold_result"
            )
...
def quick_f_check(
    ...
    if ok:
        ...
        message = (
            f"f = {f:.1f} differs from expected {expected_f:.1f} by {rel_diff*100:.0f}%"
        )
