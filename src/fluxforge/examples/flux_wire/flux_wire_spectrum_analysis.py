...
PROCESSED_DIR = (
    ROOT_DIR / "rafm_irradiation_ldrd" / "irradiation_QG_processed" / "flux_wires"
)
...
WIRE_METADATA = [
    ...
    },
    {
        ...
        "e_start": 6.82e-7,
        "e_end": 7.79e-2,
        "reaction": "In-113(n,g)In-114m [Cd]",
        "category": "epithermal",
    },
    {
        "sample": "In-Cd-RAFM-2",
    ...
    },
    {
        ...
        "e_start": 4.97e-7,
        "e_end": 3.50e-5,
        "reaction": "In-115(n,g)In-116m [Cd]",
        "category": "epithermal",
    },
    {
        "sample": "In-Cd-RAFM-2",
...
IRDFF_ENERGY_GROUPS = {
    ...
    "low_epithermal": {
        "e_low": 0.5e-6,
        "e_high": 1e-3,
        "label": "Low Epithermal",
    },  # 0.5 eV - 1 keV
    "high_epithermal": {
        "e_low": 1e-3,
        "e_high": 0.1,
        "label": "High Epithermal",
    },  # 1 keV - 100 keV
...
# ==============================================================================
def correct_activity_to_eoi(
    activity_at_measurement: float, half_life_s: float, decay_time_s: float
) -> float:
    """
...
def get_decay_time_from_sample(
    sample_name: str, measurement_time: Optional[datetime] = None
) -> float:
    """
...
def calculate_flux_categories(flux_df: pd.DataFrame) -> Dict[str, float]:
    ...
    return {
        ...
        "total": (
            np.nanmean([thermal, epithermal, fast])
            if not all(np.isnan([thermal, epithermal, fast]))
            else 0.0
        ),
    }
...
def calculate_ti_consistency(flux_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    cv = (
        (std_flux / mean_flux * 100) if mean_flux > 0 else 0
    ...
    return {
        ...
        "max_deviation_percent": (
            100 * np.max(np.abs(flux_values - mean_flux)) / mean_flux
            if mean_flux > 0
            else 0
        ),
...
def parse_processed_file(filepath: Path) -> ProcessedSpectrum:
    ...
    for line in lines:
        ...
        if iso_match and "Activity" in line:
            ...
            act_match = re.search(
                r"Activity\s*=\s*([\d.E+-]+)\s*[�±]\s*([\d.E+-]+)\s*(\w+)", line
            )
            if act_match:
...
def detect_peaks_raw(
    ...
    peaks, properties = find_peaks(
        counts_smooth, prominence=prominence_threshold, width=width_range, distance=5
    )
...
def identify_peaks(
    peaks: List[Dict], tolerance_keV: float = 2.0, sample_name: str = ""
) -> List[Dict]:
    """
...
def plot_overlay_spectra(
    ...
    sample_name_for_peaks: Optional[str] = None,
) -> None:
    """
    ...
    for name, spec in spectra.items():
        ...
        count_rate = (
            spec.counts[mask] / spec.live_time
            if spec.live_time > 0
            else spec.counts[mask]
        )
    ...
    if annotate_peaks:
        ...
        for spec in spectra.values():
            mask_s = (spec.energies >= energy_range[0]) & (
                spec.energies <= energy_range[1]
            )
            if np.any(mask_s):
                cr = (
                    spec.counts[mask_s] / spec.live_time
                    if spec.live_time > 0
                    else spec.counts[mask_s]
                )
            ...
            if iso in GAMMA_ENERGIES:
                for gamma_e in GAMMA_ENERGIES[iso]:
                    if energy_range[0] <= gamma_e <= energy_range[1]:
                        ...
                        too_close = any(
                            abs(gamma_e - ae) < 20 for ae in annotated_energies
                        )
                        if not too_close:
                            ax.axvline(
                                gamma_e,
...
def plot_single_spectrum_annotated(
    spectrum: RawSpectrum,
    output_path: Path,
    ...
    mask = (spectrum.energies >= energy_range[0]) & (
        spectrum.energies <= energy_range[1]
    )

    ax.plot(
        spectrum.energies[mask],
...
def plot_ti_variants_comparison(
    spectra: Dict[str, RawSpectrum],
    output_path: Path,
    energy_range: Tuple[float, float] = (50, 1500),
) -> None:
    ...
    if base_name in ti_variants:
        ...
        base_mask = (base_spec.energies >= energy_range[0]) & (
            base_spec.energies <= energy_range[1]
        )
        ...
        for i, (name, spec) in enumerate(sorted(ti_variants.items())):
            if name == base_name:
                continue
            mask = (spec.energies >= energy_range[0]) & (
                spec.energies <= energy_range[1]
            )
            rate = (
                spec.counts[mask] / spec.live_time
...
def plot_processed_vs_raw_comparison(
    ...
    for sample_name, proc in processed.items():
        ...
        if raw_name not in raw_peaks:
            ...
            for key in raw_peaks.keys():
                if (
                    sample_name.replace("_", "-") in key
                    or key.replace("_", "-") in sample_name
                ):
                    raw_name = key
                    break
        ...
        for iso_data in proc.isotopes:
            ...
            for peak in peaks:
                ...
                if (
                    peak_iso.lower() == isotope.lower()
                    or peak_iso.lower() == isotope.lower().replace("m", "")
                ):
    ...
    if not comparison_data:
        ...
        ax.text(
            0.5,
            0.5,
            "No matching data found between processed and raw analysis",
            ha="center",
            va="center",
    ...
    if len(df) > 1:
        ...
        # Fit quality
        r_squared = 1 - np.sum(
            (log_raw - (coeffs[0] * log_proc + coeffs[1])) ** 2
        ) / np.sum((log_raw - log_raw.mean()) ** 2)
        summary_text += f"\nLinear Fit (log-log):\n"
...
def plot_flux_summary(flux_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for cat in flux_df["category"].unique():
        ...
        ax3.scatter(
            cat_df["cross_section_b"], cat_df["flux"], label=cat, s=60, alpha=0.7
        )
...
def plot_energy_spectrum_validation(
    output_path: Path,
    flux_df: Optional[pd.DataFrame] = None,
    mcnp_spectrum: Optional[MCNPSpectrum] = None,
) -> None:
    """
    ...
    if mcnp_spectrum is not None:
        ...
        ax1.fill_between(
            mcnp_spectrum.energy_mid * 1e6,  # Convert to eV
        ...
        ax1.loglog(
            mcnp_spectrum.energy_mid * 1e6,
            mcnp_spectrum.lethargy_flux,
            "b-",
            linewidth=2,
            label="MCNP Calculated Spectrum",
        )
    else:
        ...
        ax1.loglog(
            e_centers, total_flux, "b-", linewidth=2, label="Mock Maxwellian + Fission"
        )
    ...
    if flux_df is not None and len(flux_df) > 0:
        ...
        for d in exp_data:
            color = {"thermal": "red", "epithermal": "orange", "fast": "green"}[
                d["cat"]
            ]
    ...
    ax1.set_title(
        "Neutron Spectrum Validation: MCNP vs Flux Wire Measurements\n(UWNR TRIGA Whale Bottle Position)"
    )
...
# ==============================================================================
def unfold_spectrum_multigroup(
    flux_df: pd.DataFrame,
    mcnp_spectrum: MCNPSpectrum,
    energy_groups: Optional[Dict] = None,
    ...
    for group_name, group_info in energy_groups.items():
        ...
        measured_unc = (
            group_reactions["flux"].std()
            if len(group_reactions) > 1
            else measured_flux * 0.15
        )
...
def plot_reactor_spectrum_reconstruction(
    flux_df: pd.DataFrame,
    output_path: Path,
    mcnp_spectrum: Optional[MCNPSpectrum] = None,
) -> None:
    ...
    norm_factor = (
        total_measured_flux / (total_mcnp_flux / len(mcnp_spectrum.flux))
        if total_mcnp_flux > 0
        else 1.0
    )
    ...
    ax_main.text(
        1e-2, 3e14, "Thermal", fontsize=11, color="darkblue", fontweight="bold"
    )
    ax_main.text(
        1e2, 3e14, "Epithermal", fontsize=11, color="darkorange", fontweight="bold"
    )
    ...
    ax_main.set_title(
        "Unfolded Neutron Flux Spectrum in Whale Bottle (UWNR TRIGA at 1 MW)\n"
        "Chiesa et al. 2020 IRDFF-II Methodology",
        fontsize=14,
        fontweight="bold",
    )
    ...
    thermal_unfolded = (
        np.sum(unfolded_flux[thermal_mask]) if np.any(thermal_mask) else 0
    )
    epithermal_unfolded = (
        np.sum(unfolded_flux[epithermal_mask]) if np.any(epithermal_mask) else 0
    )
    ...
    summary_text += (
        f"  Epithermal:  {len(flux_df[flux_df['category']=='epithermal'])}\n"
    )
...
def load_rafm3_sample_data(rafm3_dir: Path = RAFM3_DATA_DIR) -> Dict[str, pd.DataFrame]:
    ...
    for txt_file in sorted(rafm3_dir.glob("*.txt")):
        try:
            ...
            decay_time = get_decay_time_from_sample(
                txt_file.stem, proc.measurement_date
            )
...
EUROFER_REACTIONS = [
    ...
    },
    {
        ...
        "product": "mn56",
        "xs": 0.0001,
        "reaction": "Fe-56(n,p)Mn-56",
        "category": "fast",
    },  # (n,p) threshold reaction
    {
        "element": "Cr",
        "target": "cr50",
        "product": "cr51",
        "xs": 15.9,
        "reaction": "Cr-50(n,g)Cr-51",
        "category": "thermal",
    },
    {
        "element": "Mn",
...
def analyze_rafm_as_flux_wire(
    rafm_activities: Dict[str, float],
    sample_mass_g: float,
    irr_time_s: float = 7200,
    decay_time_s: float = 86400,
) -> pd.DataFrame:
    """
...
def run_full_analysis():
    ...
    if mcnp_spectrum is not None:
        ...
        print(
            f"    Energy range: {mcnp_spectrum.energy_low[0]*1e6:.2e} eV to {mcnp_spectrum.energy_high[-1]*1e6:.2e} eV"
        )
    ...
    for name in sorted(raw_spectra.keys()):
        ...
        print(
            f"      - {name}: {len(spec.channels)} channels, LT={spec.live_time:.0f}s"
        )
    ...
    for name, spec in raw_spectra.items():
        ...
        print(
            f"      - {name}: {len(peaks)} peaks, {n_identified} ID'd ({n_expected} expected isotopes)"
        )
    ...
    plot_ti_variants_comparison(raw_spectra, OUTPUT_DIR / "ti_variants_comparison.png")
    ...
    plot_processed_vs_raw_comparison(
        processed, raw_peaks, OUTPUT_DIR / "processed_vs_raw_comparison.png"
    )
    ...
    if flux_csv.exists():
        ...
        for i, row in flux_df.iterrows():
            for meta in WIRE_METADATA:
                if meta["prod"] == row.get("product", ""):
                    flux_df.loc[i, "cross_section_b"] = (
                        meta["xs"] if meta["unit"] == "b" else meta["xs"] * 1e-3
                    )
        ...
        if bare_activities and cd_activities:
            ...
            print(
                f"    f (thermal/epithermal): {k0_params.f:.1f} ± {k0_params.f_uncertainty:.1f}"
            )
            print(
                f"    α (epithermal deviation): {k0_params.alpha:.3f} ± {k0_params.alpha_uncertainty:.3f}"
            )
    ...
    if rafm3_data:
        ...
        if all_rafm3:
            ...
            combined_rafm3.to_csv(
                OUTPUT_DIR / "rafm3_combined_activities.csv", index=False
            )
            print(f"    Combined RAFM3 data saved: {len(combined_rafm3)} measurements")

            # Summarize by isotope
            iso_summary = (
                combined_rafm3.groupby("isotope")
                .agg(
                    {
    ...
    if rafm3_data:
        ...
        if best_file and best_file in rafm3_data:
            ...
            rafm_activities = {
                row["isotope"].capitalize(): row["activity_eoi_Bq"]
    ...
    if len(rafm_flux_df) > 0:
        ...
        print(
            rafm_flux_df[["reaction", "category", "activity_Bq", "flux"]].to_string(
                index=False
            )
        )
    ...
    create_validation_table(
        flux_df, mcnp_spectrum, OUTPUT_DIR / "validation_summary.csv"
    )
    ...
    for data_file in sorted(OUTPUT_DIR.glob("*.csv")) + sorted(
...
def create_validation_table(
    flux_df: pd.DataFrame, mcnp_spectrum: Optional[MCNPSpectrum], output_path: Path
) -> None:
    """
