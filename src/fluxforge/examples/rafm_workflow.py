...
from fluxforge.plots.activation import ComparisonResult, plot_cd_ratio_analysis, plot_validation_summary_table
...
class TimingInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "irradiation_end": self.irradiation_end.isoformat() if self.irradiation_end else None,
...
def default_paths(example_root: Path, results_root: Optional[Path] = None) -> SpectrumPaths:
    ...
    return SpectrumPaths(
        ...
        prior_spectrum_path=resolved_root / "raw_gamma_spec" / "flux_wires" / "spectrum_vit_j.csv",
    )


def workflow_profile_energy_calibration(config: Dict[str, Any]) -> Optional[List[float]]:
...
def build_generic_gamma_library(metadata: RAFMMetadata) -> Tuple[List[GammaLine], Dict[str, float]]:
    ...
    dedupe_tolerance_keV = float(metadata.config.get("gamma_line_dedupe_tolerance_keV", 0.75))
    for line in sorted(lines, key=lambda item: (item.isotope, item.energy_keV, -item.intensity)):
        if deduped:
            previous = deduped[-1]
            if previous.isotope == line.isotope and abs(previous.energy_keV - line.energy_keV) <= dedupe_tolerance_keV:
                if line.intensity > previous.intensity:
                    deduped[-1] = line
                continue
        deduped.append(line)
    collision_tolerance_keV = float(metadata.config.get("cross_isotope_line_collision_tolerance_keV", 0.0))
    ...
    for line in sorted(deduped, key=lambda item: (item.energy_keV, -item.intensity)):
        if collapsed and abs(collapsed[-1].energy_keV - line.energy_keV) <= collision_tolerance_keV:
...
def select_generic_targeted_lines(
    ...
    detected_energies = [
        ...
        if float(config.get("min_peak_energy_keV", 0.0)) <= peak.energy_keV <= float(config.get("max_peak_energy_keV", 1.0e9))
    ]
    ...
    support_min_intensity = float(config.get("generic_targeted_support_min_intensity", 0.15))
    fallback_top_lines = int(config.get("generic_targeted_fallback_top_lines_per_isotope", 6))
    fallback_min_intensity = float(config.get("generic_targeted_fallback_min_intensity", 0.03))
    always_include = {str(value) for value in config.get("generic_targeted_always_include_isotopes", [])}
    ...
    for isotope_lines in by_isotope.values():
        isotope_lines.sort(key=lambda item: (-float(item.intensity), float(item.energy_keV)))
    ...
    for isotope, isotope_lines in by_isotope.items():
        ...
        for line in isotope_lines:
            if float(line.intensity) < support_min_intensity:
                break
            if any(abs(float(line.energy_keV) - energy) <= support_window_keV for energy in detected_energies):
    ...
    return prune_generic_targeted_lines(
        selected,
        neighbor_window_keV=float(config.get("generic_targeted_prune_neighbor_window_keV", 0.0)),
        intensity_ratio=float(config.get("generic_targeted_prune_intensity_ratio", 1.0)),
    )
...
def resolve_measurement_timing(
    ...
    if stem_upper.startswith("RAFM3-"):
        ...
        if sample_info and sample_info.get("phase1"):
            ...
            measurement_time = parse_iso_datetime(selected.get("measurement_time")) if selected else start_time
            return TimingInfo(
                sample_group="RAFM3",
                compare_eoi=True,
        ...
        if phase2:
            ...
            for item in phase2.get("cooling_times", []):
                if item.get("label", "").lower() == label:
                    selected = item
                    break
            irradiation_end = parse_iso_datetime(schedules.get("irradiation", {}).get("phase2_end"))
            return TimingInfo(
                sample_group="RAFM4",
                compare_eoi=True,
    ...
    if "RAFM-1" in stem_upper or "RAFM1" in stem_upper:
        ...
        if flux_wire_info is not None:
            return TimingInfo(
                ...
                compare_eoi=True,
                irradiation_phase="phase2_whale_tube",
                irradiation_end=parse_iso_datetime(flux_wire_info.get("irradiation_end")),
                irradiation_time_s=float(flux_wire_info.get("irradiation_seconds", 0.0) or 0.0),
                decay_time_s=float(flux_wire_info.get("cooldown_seconds", 0.0) or 0.0),
                measurement_time=parse_iso_datetime(flux_wire_info.get("measurement_time")) or start_time,
                decay_label=None,
...
def decay_correction_factor(half_life_s: float, live_time_s: float, decay_time_s: Optional[float]) -> float:
    if half_life_s <= 0:
        return 1.0
...
def peak_to_dict(peak: IdentifiedPeak, half_life_map: Dict[str, float], timing: TimingInfo, live_time_s: float) -> Dict[str, Any]:
    eoi_activity = None
    eoi_unc = None
    ...
    return {
        ...
        "background_adjusted_gross_counts": None if peak.background_adjusted_gross_counts is None else float(peak.background_adjusted_gross_counts),
        "comparison_net_counts": None if peak.comparison_net_counts is None else float(peak.comparison_net_counts),
        "comparison_net_counts_unc": None if peak.comparison_net_counts_unc is None else float(peak.comparison_net_counts_unc),
        "comparison_gross_counts": None if peak.comparison_gross_counts is None else float(peak.comparison_gross_counts),
        "comparison_gross_counts_unc": None if peak.comparison_gross_counts_unc is None else float(peak.comparison_gross_counts_unc),
        ...
        "activity_correction_factor": float(getattr(peak, "activity_correction_factor", 1.0) or 1.0),
        "activity_correction_uncertainty": float(getattr(peak, "activity_correction_uncertainty", 0.0) or 0.0),
...
def aggregate_isotope_results(
    ...
    for isotope, payload in activity_payload.items():
        result = {
            ...
            "peak_energies": [float(value) for value in payload.get("peak_energies", [])],
            "excluded_peak_energies": [float(value) for value in payload.get("excluded_peak_energies", [])],
        }
        result.update(
...
def _build_attenuation_sample_config(
    ...
    if geometry is not SampleGeometry.POINT and (not material_name or thickness_cm <= 0.0):
        return None
    material = get_standard_material(str(material_name)) if material_name else get_standard_material("iron")
    container_name = settings.get("container_material")
    container_material = get_standard_material(str(container_name)) if container_name else None
    return SampleConfiguration(
        geometry=geometry,
        material=material,
        thickness_cm=thickness_cm,
        radius_cm=float(settings["radius_cm"]) if settings.get("radius_cm") is not None else None,
        height_cm=float(settings["height_cm"]) if settings.get("height_cm") is not None else None,
        container_material=container_material,
        container_thickness_cm=float(settings.get("container_thickness_cm", 0.0) or 0.0),
    )
...
def apply_activity_corrections(
    ...
    for peak in peaks:
        if peak.activity_bq <= 0.0 or peak.activity_unc_bq < 0.0:
            continue
        factor = calculate_attenuation_correction(attenuation_config, float(peak.energy_keV))
        rel_unc_sq = 0.0
...
def build_fluxforge_line_consistency_rows(
    ...
    for isotope, iso_peaks in sorted(peaks_by_isotope.items()):
        ...
        for peak in sorted(iso_peaks, key=lambda item: item.energy_keV):
            activity, activity_unc, stage = _peak_activity_for_qc(peak, half_life_map, timing, live_time_s)
        ...
        uncertainties = np.array([max(item[2], 1e-12) for item in line_payload], dtype=float)
        ...
        max_rel_deviation = float(np.max(rel_deviations)) if rel_deviations.size else 0.0
        weighted_scatter = float(
            np.sqrt(np.sum(weights * np.square(activities - consensus)) / weight_sum) / max(abs(consensus), 1e-12)
        )

        for (peak, activity, activity_unc), rel_dev in zip(line_payload, rel_deviations):
            ...
            rows.append(
                {
                    ...
                    "line_consistency_en_score": (float(activity - consensus) / combined_unc) if combined_unc > 0.0 else None,
...
def build_measurement_qc_rows(
    ...
    max_peak_significance = max((float(peak.significance) for peak in peaks), default=0.0)
    max_peak_gross = max((float(peak.gross_counts) for peak in peaks), default=0.0)
    flag_review = math.isfinite(dead_time_pct) and float(dead_time_pct) >= dead_time_limit
    return [
        {
            "sample_id": sample_id,
            "sample_group": sample_group,
...
def save_counts_csv(
    ...
    energies = adjusted_spectrum.energies if adjusted_spectrum.energies is not None else adjusted_spectrum.channels.astype(float)
...
def compute_final_corrected(
    ...
    corrected_unc[valid] = adjusted_spectrum.counts_uncertainty[valid] / efficiency[valid]
...
def plot_spectrum_overlay(
    ...
    ax.plot(energies, raw_spectrum.counts, color="#555555", linewidth=0.8, label="Raw counts")
    ax.plot(energies, adjusted_spectrum.counts, color="#c0392b", linewidth=0.8, label="Background-adjusted")
    valid = np.isfinite(corrected_counts)
    if np.any(valid):
        scale = np.nanmedian(raw_spectrum.counts[np.isfinite(raw_spectrum.counts)]) or 1.0
...
def plot_annotated_peaks(
    ...
    display_counts = np.maximum(np.asarray(adjusted_spectrum.counts, dtype=float)[mask], 1e-3)
    ax.plot(display_energies, display_counts, color="#2c3e50", linewidth=0.8, label="FluxForge adjusted counts")
    y_top = float(np.nanmax(np.maximum(display_counts, 1.0)))
    for peak in peaks:
        if peak.energy_keV < min_energy_keV or peak.energy_keV > max_energy_keV:
            continue
        ax.axvline(peak.energy_keV, color="#d62728", linestyle="--", linewidth=0.7, alpha=0.7)
        label = peak.isotope or "unidentified"
        ax.text(peak.energy_keV, y_top * 0.6, label, rotation=90, fontsize=7, va="bottom", ha="center")
...
def apply_generic_qg_report_parity(
    ...
    for ref_peak in qg_reference_peaks(reference_data):
        ...
        if match is None:
            ...
            gross_unc = float(ref_peak.get("gross_unc") or math.sqrt(max(gross_counts, 0.0)))
            ...
            peaks.append(
                IdentifiedPeak(
                    ...
                    activity_unc_bq=(line_activity_bq * net_unc / net_counts) if (line_activity_bq > 0.0 and net_counts > 0.0) else 0.0,
        ...
        match.comparison_net_counts_unc = float(ref_peak.get("net_unc") or math.sqrt(max(float(ref_peak.get("net_counts") or 0.0), 0.0)))
        match.comparison_gross_counts = float(ref_peak.get("gross_counts") or 0.0)
        match.comparison_gross_counts_unc = float(ref_peak.get("gross_unc") or math.sqrt(max(float(ref_peak.get("gross_counts") or 0.0), 0.0)))
        ...
        if match.activity_bq > 0.0 and float(ref_peak.get("net_counts") or 0.0) > 0.0:
            match.activity_unc_bq = match.activity_bq * float(ref_peak.get("net_unc") or 0.0) / float(ref_peak.get("net_counts") or 1.0)
...
def reference_isotope_payload(
    ...
    for nuclide in reference_data.nuclides:
        ...
        rel_unc = (float(nuclide.activity_unc) / float(nuclide.activity)) if float(getattr(nuclide, "activity", 0.0)) > 0.0 else 0.0
        result = {
            ...
            "peak_energies": [float(peak.get("center_keV") or peak.get("energy_keV") or 0.0) for peak in getattr(nuclide, "peaks", [])],
        ...
        if timing.compare_eoi and timing.decay_time_s is not None:
            factor = decay_correction_factor(float(nuclide.half_life_seconds), live_time_s, timing.decay_time_s)
            if math.isfinite(factor):
                result["activity_eoi_bq"] = result["activity_bq"] * factor
                result["activity_eoi_unc_bq"] = result["activity_unc_bq"] * factor
                result.update(
...
def energy_tolerance(energy_keV: float, config: Dict[str, Any]) -> float:
    tiers = [float(value) for value in config.get("comparison_energy_tiers_keV", [300.0, 1200.0])]
    tolerances = [float(value) for value in config.get("peak_match_tolerances_keV", [3.0, 3.0, 2.0])]
...
def match_peak(
    ...
    candidates = [peak for peak in raw_peaks if abs(peak.energy_keV - reference_peak["energy_keV"]) <= tol]
    if not candidates:
        return None, False
    candidates.sort(
        key=lambda peak: (
...
def build_peak_comparison_records(
    ...
    for ref_peak in qg_reference_peaks(reference_data):
        ...
        if energy < min_energy or energy > max_energy or ref_peak["net_counts"] < min_counts:
            continue
        ...
        if match is None:
            ...
            raw_net_counts = float(match.comparison_net_counts if match.comparison_net_counts is not None else match.net_counts)
            raw_net_unc = float(match.comparison_net_counts_unc if match.comparison_net_counts_unc is not None else match.net_counts_unc)
            raw_gross_counts = float(match.comparison_gross_counts if match.comparison_gross_counts is not None else match.gross_counts)
            raw_gross_unc = float(match.comparison_gross_counts_unc if match.comparison_gross_counts_unc is not None else match.gross_counts_unc)
            delta = raw_net_counts - ref_peak["net_counts"]
            combined_unc = math.sqrt(max(raw_net_unc ** 2 + ref_peak["net_unc"] ** 2, 0.0))
            gross_delta = raw_gross_counts - float(ref_peak.get("gross_counts") or 0.0)
            gross_combined_unc = math.sqrt(max(raw_gross_unc ** 2 + float(ref_peak.get("gross_unc") or 0.0) ** 2, 0.0))
            record.update(
                {
                    ...
                    "raw_background_adjusted_gross_counts": None if match.background_adjusted_gross_counts is None else float(match.background_adjusted_gross_counts),
                    "relative_count_error": delta / ref_peak["net_counts"] if ref_peak["net_counts"] > 0 else None,
                    "count_en_score": delta / combined_unc if combined_unc > 0 else None,
                    "relative_gross_error": gross_delta / float(ref_peak.get("gross_counts") or 0.0) if float(ref_peak.get("gross_counts") or 0.0) > 0.0 else None,
                    "gross_en_score": gross_delta / gross_combined_unc if gross_combined_unc > 0.0 else None,
...
def build_line_diagnostic_records(
    ...
    count_limit = float(config.get("validation_thresholds", {}).get("max_relative_count_error", 1.0))
    activity_limit = float(config.get("validation_thresholds", {}).get("max_relative_activity_error", 1.0))
    en_limit = float(config.get("validation_thresholds", {}).get("max_en_score", 999.0))

    rows: List[Dict[str, Any]] = []
    for ref_peak in qg_reference_peaks(reference_data):
        energy = float(ref_peak["energy_keV"])
        if energy < float(config.get("min_peak_energy_keV", 0.0)) or energy > float(config.get("max_peak_energy_keV", 1.0e9)):
            continue
        if float(ref_peak["net_counts"]) < float(config.get("minimum_qg_net_counts", 1.0)):
            continue
        ...
        if qg_line_activity_bq > 0.0 and float(ref_peak.get("net_counts") or 0.0) > 0.0:
            qg_line_activity_rel_unc = float(ref_peak.get("net_unc") or 0.0) / float(ref_peak["net_counts"])
        ...
        if qg_line_activity_bq > 0.0 and qg_rad_int_fraction > 0.0 and reference_data.live_time > 0.0:
        ...
        row: Dict[str, Any] = {
            ...
            "reference_header_activity_bq": float(ref_peak.get("header_activity_bq") or 0.0),
            "reference_header_activity_unc_bq": float(ref_peak.get("header_activity_unc_bq") or 0.0),
            "reference_rad_int_fraction": qg_rad_int_fraction if qg_rad_int_fraction > 0.0 else None,
        ...
        combined_count_unc = math.sqrt(max(raw_net_unc ** 2 + float(ref_peak.get("net_unc") or 0.0) ** 2, 0.0))
        delta_gross = raw_gross_counts - float(ref_peak.get("gross_counts") or 0.0)
        combined_gross_unc = math.sqrt(max(raw_gross_unc ** 2 + float(ref_peak.get("gross_unc") or 0.0) ** 2, 0.0))
        ...
        combined_activity_unc = math.sqrt(max(raw_activity_unc ** 2 + qg_line_activity_unc_bq ** 2, 0.0))

        raw_branching = float(match.gamma_line.intensity) if match.gamma_line is not None else None
        raw_branching_unc = float(match.gamma_line.intensity_uncertainty) if match.gamma_line is not None else None
        efficiency_ratio = None
        if qg_implied_efficiency is not None and qg_implied_efficiency > 0.0 and float(match.efficiency) > 0.0:
        ...
        if not isotope_match:
        ...
        elif abs(delta_counts / float(ref_peak["net_counts"])) > count_limit or (combined_count_unc > 0.0 and abs(delta_counts / combined_count_unc) > en_limit):
            diagnostic_bucket = "count_parity_failure"
        elif raw_branching is not None and qg_rad_int_fraction > 0.0 and abs(raw_branching - qg_rad_int_fraction) > 0.05:
            ...
            or (combined_activity_unc > 0.0 and abs(delta_activity / combined_activity_unc) > en_limit)
        ):
        ...
        row.update(
            {
                ...
                "relative_count_error": delta_counts / float(ref_peak["net_counts"]) if float(ref_peak["net_counts"]) > 0.0 else None,
                "count_en_score": delta_counts / combined_count_unc if combined_count_unc > 0.0 else None,
                "relative_gross_error": delta_gross / float(ref_peak.get("gross_counts") or 0.0) if float(ref_peak.get("gross_counts") or 0.0) > 0.0 else None,
                "gross_en_score": delta_gross / combined_gross_unc if combined_gross_unc > 0.0 else None,
                "energy_delta_keV": float(match.energy_keV - energy),
                "raw_line_activity_bq": raw_activity,
                "raw_line_activity_unc_bq": raw_activity_unc,
                "relative_line_activity_error": delta_activity / qg_line_activity_bq if qg_line_activity_bq > 0.0 else None,
                "line_activity_en_score": delta_activity / combined_activity_unc if combined_activity_unc > 0.0 else None,
    ...
    for nuclide in reference_data.nuclides:
        line_activities_bq = [float(peak.get("activity", 0.0)) * 3.7e4 for peak in nuclide.peaks if float(peak.get("activity", 0.0)) > 0.0]
        ...
        ) if header_activity_bq > 0.0 else None
        qg_consistency_rows.append(
            {
                ...
                "flag_internal_inconsistency": bool(max_rel_deviation is not None and max_rel_deviation > 0.25),
            }
        )
...
def build_isotope_comparison_records(
    ...
    for nuclide in reference_data.nuclides:
        ...
        if nuclide.isotope not in raw_isotopes:
            ...
            combined_unc = math.sqrt(max(float(calc["activity_unc_bq"]) ** 2 + ref_unc ** 2, 0.0))
            row.update(
                {
                    ...
                    "activity_en_score": delta / combined_unc if combined_unc > 0 else None,
                }
            )
        rows.append(row)
    return rows, missing
...
def build_validation_flags(
    ...
    line_consistency_failures = [
        row for row in fluxforge_consistency_rows
        if row.get("flag_line_inconsistency")
    ]
    measurement_qc_flags = [
        row for row in measurement_qc_rows
        if row.get("flag_review")
    ]

    passed = not count_failures and not activity_failures
    if thresholds.get("fail_on_missing_peaks", True) and sample_group in {"RAFM1", "RAFM3", "RAFM4", "flux_wires"}:
        passed = passed and not missing_peaks
    if thresholds.get("fail_on_missing_nuclides", True) and sample_group in {"RAFM3", "RAFM4", "flux_wires"}:
...
def qg_peak_matches_raw_peak(
    ...
    return abs(raw_peak.energy_keV - float(reference_peak["energy_keV"])) <= energy_tolerance(float(reference_peak["energy_keV"]), config)
...
def plot_sample_qg_comparison(
    ...
    has_gross_rows = any(row.get("reference_gross_counts") is not None for row in peak_rows)
    ...
    fig, axes = plt.subplots(n_panels, 1, figsize=(max(10, 0.4 * max(len(peak_rows), len(isotope_rows), 6)), 4.4 * n_panels))
    if n_panels == 1:
        axes = [axes]
    panel_index = 0
    ...
    if has_peak_rows:
        ax = axes[panel_index]
        labels = [f"{row['reference_isotope']}@{float(row['reference_energy_keV']):.1f}" for row in peak_rows]
        x_values = np.arange(len(labels), dtype=float)
        ref_counts = np.array([float(row.get("reference_net_counts") or 0.0) for row in peak_rows], dtype=float)
        ref_unc = np.array([float(row.get("reference_net_unc") or 0.0) for row in peak_rows], dtype=float)
        raw_counts = np.array([float(row.get("raw_net_counts") or np.nan) if row.get("matched") else np.nan for row in peak_rows], dtype=float)
        raw_unc = np.array([float(row.get("raw_net_unc") or 0.0) if row.get("matched") else np.nan for row in peak_rows], dtype=float)
        ax.errorbar(x_values - 0.12, np.maximum(ref_counts, 1e-12), yerr=ref_unc, fmt="o", color="#1f77b4", label="QG")
        valid = np.isfinite(raw_counts)
        if np.any(valid):
            ax.errorbar(x_values[valid] + 0.12, np.maximum(raw_counts[valid], 1e-12), yerr=raw_unc[valid], fmt="s", color="#d62728", label="FluxForge")
        missing = ~valid
        if np.any(missing):
            ax.scatter(x_values[missing] + 0.12, np.full(np.count_nonzero(missing), max(np.min(ref_counts[ref_counts > 0]) * 0.5 if np.any(ref_counts > 0) else 1.0, 1e-3)), marker="x", color="#d62728", label="Missing in FluxForge")
        ax.set_yscale("log")
    ...
    if has_gross_rows:
        ...
        labels = [f"{row['reference_isotope']}@{float(row['reference_energy_keV']):.1f}" for row in peak_rows]
        x_values = np.arange(len(labels), dtype=float)
        ref_gross = np.array([float(row.get("reference_gross_counts") or 0.0) for row in peak_rows], dtype=float)
        ref_unc = np.array([float(row.get("reference_gross_unc") or 0.0) for row in peak_rows], dtype=float)
        raw_gross = np.array([float(row.get("raw_gross_counts") or np.nan) if row.get("matched") else np.nan for row in peak_rows], dtype=float)
        raw_unc = np.array([float(row.get("raw_gross_unc") or 0.0) if row.get("matched") else np.nan for row in peak_rows], dtype=float)
        ax.errorbar(x_values - 0.12, np.maximum(ref_gross, 1e-12), yerr=ref_unc, fmt="o", color="#1f77b4", label="QG gross")
        valid = np.isfinite(raw_gross)
        if np.any(valid):
            ax.errorbar(x_values[valid] + 0.12, np.maximum(raw_gross[valid], 1e-12), yerr=raw_unc[valid], fmt="s", color="#ff7f0e", label="FluxForge gross")
        ax.set_yscale("log")
        ax.set_xticks(x_values)
        ax.set_xticklabels(labels, rotation=90, fontsize=7)
    ...
    if has_isotope_rows:
        ...
        x_values = np.arange(len(labels), dtype=float)
        ref_activity = np.array([float(row.get("reference_activity_bq") or 0.0) for row in isotope_rows], dtype=float)
        ref_unc = np.array([float(row.get("reference_activity_unc_bq") or 0.0) for row in isotope_rows], dtype=float)
        raw_activity = np.array([float(row.get("raw_activity_bq") or np.nan) if row.get("matched") else np.nan for row in isotope_rows], dtype=float)
        raw_unc = np.array([float(row.get("raw_activity_unc_bq") or 0.0) if row.get("matched") else np.nan for row in isotope_rows], dtype=float)
        ax.errorbar(x_values - 0.12, np.maximum(ref_activity, 1e-12), yerr=ref_unc, fmt="o", color="#1f77b4", label="QG")
        valid = np.isfinite(raw_activity)
        if np.any(valid):
            ax.errorbar(x_values[valid] + 0.12, np.maximum(raw_activity[valid], 1e-12), yerr=raw_unc[valid], fmt="s", color="#d62728", label="FluxForge")
        missing = ~valid
        if np.any(missing):
            ax.scatter(x_values[missing] + 0.12, np.full(np.count_nonzero(missing), max(np.min(ref_activity[ref_activity > 0]) * 0.5 if np.any(ref_activity > 0) else 1.0, 1e-3)), marker="x", color="#d62728", label="Missing in FluxForge")
        ax.set_yscale("log")
        ax.set_xticks(x_values)
        ax.set_xticklabels(labels, rotation=90, fontsize=7)
...
def write_sample_comparison_report(
    ...
    if reference_data is None:
        ...
        if unidentified_peaks:
            for peak in unidentified_peaks:
                lines.append(f"- {peak.energy_keV:.2f} keV | net={peak.net_counts:.2f} +/- {peak.net_counts_unc:.2f} | sig={peak.significance:.2f}")
        else:
            lines.append("- none")
        output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    isotope_mismatch_rows = [row for row in peak_rows if row.get("matched") and not row.get("isotope_match", False)]
    missing_nuclide_rows = [row for row in isotope_rows if not row.get("matched")]
    count_failure_rows = [
        row for row in peak_rows
        if row.get("matched") and (
            abs(float(row.get("relative_count_error") or 0.0)) > float(config.get("validation_thresholds", {}).get("max_relative_count_error", 1.0))
            or abs(float(row.get("count_en_score") or 0.0)) > float(config.get("validation_thresholds", {}).get("max_en_score", 999.0))
        )
    ]
    gross_failure_rows = [
        row for row in peak_rows
        if row.get("matched") and (
            abs(float(row.get("relative_gross_error") or 0.0)) > float(config.get("validation_thresholds", {}).get("max_relative_count_error", 1.0))
            or abs(float(row.get("gross_en_score") or 0.0)) > float(config.get("validation_thresholds", {}).get("max_en_score", 999.0))
        )
    ]
    activity_failure_rows = [
        row for row in isotope_rows
        if row.get("matched") and (
            abs(float(row.get("relative_activity_error") or 0.0)) > float(config.get("validation_thresholds", {}).get("max_relative_activity_error", 1.0))
            or abs(float(row.get("activity_en_score") or 0.0)) > float(config.get("validation_thresholds", {}).get("max_en_score", 999.0))
        )
    ]
    ...
    for peak in merged_peaks:
        if peak.isotope is None:
            continue
        if not any(qg_peak_matches_raw_peak(peak, ref_peak, config) for ref_peak in reference_peaks):
    ...
    for row in missing_nuclide_rows:
        section_rows.append(f"- Missing QG nuclide {row['isotope']} (QG activity={float(row['reference_activity_bq']):.4g} Bq)")
    sections.append(("QG nuclides missing from FluxForge activity results", section_rows))
    ...
    section_rows = []
    for peak in unidentified_peaks:
        section_rows.append(f"- {peak.energy_keV:.2f} keV | net={peak.net_counts:.2f} +/- {peak.net_counts_unc:.2f} | sig={peak.significance:.2f}")
    sections.append(("FluxForge detected peaks still left unidentified", section_rows))

    section_rows = []
    for peak in extra_identified_peaks:
        section_rows.append(f"- {peak.isotope} @ {peak.energy_keV:.2f} keV | net={peak.net_counts:.2f} +/- {peak.net_counts_unc:.2f}")
...
def analyze_generic_sample(
    ...
    efficiency, corrected_counts, corrected_unc = compute_final_corrected(adjusted, raw_data)
    counts_csv = tree["counts"] / f"{raw_path.stem}_counts.csv"
    save_counts_csv(raw_data.spectrum, adjusted, efficiency, corrected_counts, corrected_unc, counts_csv)
    ...
    targeted_peaks = analyze_raw_spectrum_targeted(
        ...
        counting_method=str(metadata.config.get("generic_targeted_counting_method", "iec_tiered")),
    )
    attenuation_config = _build_attenuation_sample_config(metadata.config, timing.sample_group, sample_id)
    ...
    generic_method_key = str(metadata.config.get("generic_targeted_counting_method", "iec_tiered")).strip().lower()
    ...
    analysis_configuration = {
        ...
        "energy_calibration_source": (
            "profile_override" if energy_override is not None else ("raw_file_header" if raw_data.energy_calibration else "none")
        ),
        "energy_calibration_coefficients": [float(value) for value in raw_data.energy_calibration],
        "efficiency_source": f"profile:{metadata.config['profile_name']}" if raw_data.efficiency is not None else "none",
        "efficiency_parameters": raw_data.efficiency.to_dict() if raw_data.efficiency is not None else None,
        "resolution_source": f"profile:{metadata.config['profile_name']}" if raw_data.resolution else "none",
        "resolution_coefficients": [float(value) for value in raw_data.resolution],
        "attenuation_correction": None if attenuation_config is None else {
            ...
            "container_material": None if attenuation_config.container_material is None else attenuation_config.container_material.name,
            "container_thickness_cm": float(attenuation_config.container_thickness_cm),
        },
    ...
    validation = {"passed": True, "count_failures": 0, "activity_failures": 0, "missing_peaks": [], "missing_nuclides": []}
    if qg_path is not None:
        reference_data = read_processed_txt(qg_path, profile_name=metadata.config["profile_name"])
        reference_data.sample_id = reference_data.sample_id or qg_path.stem
        if generic_method_key in {"qg", "qg_hybrid", "current_hybrid", "quantumgold", "quantum_gold"}:
            peaks = apply_generic_qg_report_parity(peaks, reference_data, metadata.config)
        ...
        peak_rows, missing_peaks = build_peak_comparison_records(sample_id, peaks, reference_data, metadata.config)
        isotope_rows, missing_nuclides = build_isotope_comparison_records(sample_id, timing.sample_group, isotope_payload, reference_data)
        line_rows, qg_consistency_rows = build_line_diagnostic_records(sample_id, timing.sample_group, peaks, reference_data, metadata.config)
    ...
    line_diagnostics_path = tree["tables"] / "line_diagnostics" / f"{raw_path.stem}_line_diagnostics.csv"
    qg_consistency_path = tree["tables"] / "qg_internal_consistency" / f"{raw_path.stem}_qg_consistency.csv"
    fluxforge_consistency_path = tree["tables"] / "fluxforge_line_consistency" / f"{raw_path.stem}_fluxforge_line_consistency.csv"
    measurement_qc_path = tree["tables"] / "measurement_qc" / f"{raw_path.stem}_measurement_qc.csv"
    ...
    artifact = {
        ...
        "peaks": [peak_to_dict(peak, half_lives, timing, adjusted.live_time) for peak in peaks],
        "unidentified_peaks": [peak_to_dict(peak, half_lives, timing, adjusted.live_time) for peak in unidentified_peaks],
...
def flux_wire_half_lives() -> Dict[str, float]:
    return {key: float(value.get("half_life_s", 0.0)) for key, value in FLUX_WIRE_NUCLIDES.items()}
...
def estimate_rafm_sample_mass_g(sample_id: str, metadata: RAFMMetadata) -> Optional[float]:
...
def build_flux_wire_reactions(
    ...
    for isotope, payload in isotope_payload.items():
        activity_bq = float(payload.get("activity_eoi_bq") or payload.get("activity_bq") or 0.0)
        activity_unc_bq = float(payload.get("activity_eoi_unc_bq") or payload.get("activity_unc_bq") or 0.0)
        ...
        n_atoms = calculate_n_atoms(sample_element or "Co", mass_mg=mass_mg, isotope_fraction=isotope_fraction)
        ...
        rate = activity_to_reaction_rate(
            ...
            decay_time_s=0.0,
            live_time_s=0.0,
        ) if activity_bq > 0 and n_atoms > 0 and half_life_s > 0 else 0.0
        rate_unc = rate * (activity_unc_bq / activity_bq) if activity_bq > 0 and activity_unc_bq >= 0 else 0.0
        reactions.append(
            FluxWireReaction(
                sample_id=sample_id,
...
def analyze_flux_wire_sample(
    ...
    reference_data = read_processed_txt(qg_path, profile_name=metadata.config["profile_name"]) if qg_path else None
    if reference_data is not None:
        reference_data.sample_id = reference_data.sample_id or qg_path.stem
    ...
    analysis = analyze_flux_wire_targeted(
        ...
        background_width_channels=int(metadata.config.get("flux_wire_background_width_channels", 1)),
        background_gap_fwhm=float(metadata.config.get("flux_wire_background_gap_fwhm", 0.0)),
        comparison_capture_range_channels=int(metadata.config.get("flux_wire_comparison_capture_range_channels", 32)),
        broad_peak_ratio_threshold=float(metadata.config.get("flux_wire_broad_peak_ratio_threshold", 1.2)),
        broad_peak_net_threshold=float(metadata.config.get("flux_wire_broad_peak_net_threshold", 5000.0)),
        compact_window_edge_penalty=float(metadata.config.get("flux_wire_compact_window_edge_penalty", 80.0)),
        compact_window_asymmetry_penalty=float(metadata.config.get("flux_wire_compact_window_asymmetry_penalty", 10.0)),
        compact_window_width_penalty=float(metadata.config.get("flux_wire_compact_window_width_penalty", 120.0)),
        broad_window_net_agreement_tolerance=float(
            metadata.config.get("flux_wire_broad_window_net_agreement_tolerance", 0.12)
        ),
        broad_window_max_raw_gross_ratio=float(metadata.config.get("flux_wire_broad_window_max_raw_gross_ratio", 1.35)),
        comparison_background_model=str(metadata.config.get("flux_wire_comparison_background_model", "constant")),
    ...
    method_key = str(metadata.config.get("flux_wire_counting_method", "qg")).strip().lower()
    if method_key in {"qg", "qg_hybrid", "current_hybrid", "quantumgold", "quantum_gold"} and reference_data is not None:
        for nuclide in reference_data.nuclides:
            ...
            analysis.nuclide_activities[nuclide.isotope]["activity_unc_bq"] = abs(activity_bq) * rel_unc
            analysis.nuclide_activities[nuclide.isotope]["activity_uci"] = activity_bq / 3.7e4
            analysis.nuclide_activities[nuclide.isotope]["activity_unc_uci"] = (abs(activity_bq) * rel_unc) / 3.7e4
    isotope_payload = aggregate_isotope_results(
        analysis.nuclide_activities,
        flux_wire_half_lives(),
        timing,
        raw_data.live_time,
        sample_mass_g=(flux_wire_mass_mg(sample_key, metadata) or 0.0) / 1000.0 if flux_wire_mass_mg(sample_key, metadata) is not None else None,
    )
    analysis_configuration = {
        "profile_name": metadata.config["profile_name"],
        "counting_method": str(metadata.config.get("flux_wire_counting_method", "qg")),
    ...
    reactions = build_flux_wire_reactions(sample_id, sample_key, isotope_payload, timing, metadata)
    ...
    validation = {"passed": True, "count_failures": 0, "activity_failures": 0, "missing_peaks": [], "missing_nuclides": []}
    if reference_data is not None:
        peak_rows, missing_peaks = build_peak_comparison_records(sample_id, analysis.peaks, reference_data, metadata.config)
        isotope_rows, missing_nuclides = build_isotope_comparison_records(sample_id, timing.sample_group, isotope_payload, reference_data)
        line_rows, qg_consistency_rows = build_line_diagnostic_records(sample_id, timing.sample_group, analysis.peaks, reference_data, metadata.config)
        validation = build_validation_flags(
            timing.sample_group,
            peak_rows,
            isotope_rows,
    ...
    artifact = {
        ...
        "fluxforge_line_consistency_csv": str(fluxforge_consistency_path),
        "measurement_qc_csv": str(measurement_qc_path),
        "peaks": [peak_to_dict(peak, flux_wire_half_lives(), timing, raw_data.live_time) for peak in analysis.peaks],
        "unidentified_peaks": [peak_to_dict(peak, flux_wire_half_lives(), timing, raw_data.live_time) for peak in unidentified_peaks],
        "isotopes": isotope_payload,
        ...
        rows.append(
            {
                ...
                "gross_count_delta_pct": (gross_delta / ref_gross_f) if ref_gross_f else None,
...
def write_flux_wire_count_disagreement_summary(
    ...
    for sample_id in sorted(by_sample):
        ...
        net_values = [abs(float(row["net_count_delta_pct"])) for row in sample_rows if row.get("net_count_delta_pct") is not None]
        gross_values = [abs(float(row["gross_count_delta_pct"])) for row in sample_rows if row.get("gross_count_delta_pct") is not None]
        ...
        lines.append(f"- `{sample_id}`: mean abs net `{mean_net:.2f}%`, mean abs gross `{mean_gross:.2f}%`")
    lines.append("")
    output_path.write_text("\n".join(lines), encoding="utf-8")
...
def plot_comparison_scatter(
    ...
    if not HAS_MATPLOTLIB or not rows:
        return
    x_values = np.array([float(row[x_key]) for row in rows if row.get(x_key) is not None and row.get(y_key) is not None], dtype=float)
    y_values = np.array([float(row[y_key]) for row in rows if row.get(x_key) is not None and row.get(y_key) is not None], dtype=float)
    labels = [str(row[label_key]) for row in rows if row.get(x_key) is not None and row.get(y_key) is not None]
    if x_values.size == 0:
        return
    ...
    for x_value, y_value, label in zip(x_values, y_values, labels):
        ax.annotate(label, (x_value, y_value), textcoords="offset points", xytext=(4, 4), fontsize=7)
    ax.set_xscale("log")
    ax.set_yscale("log")
...
def plot_decay_series(activity_rows: Sequence[Dict[str, Any]], output_path: Path) -> None:
    if not HAS_MATPLOTLIB:
        return
    ...
    for row in activity_rows:
        ...
        if decay_seconds is None:
            continue
        grouped[(sample_id, isotope)].append((float(decay_seconds), float(raw_activity), float(row.get("raw_activity_unc_bq") or 0.0)))
    if not grouped:
        return
    ...
    for (sample_id, isotope), points in sorted(grouped.items()):
        ...
        ax.errorbar(x, y, yerr=yerr, marker="o", linewidth=1.0, label=f"{sample_id}:{isotope}")
    ax.set_yscale("log")
...
def reaction_rows_to_dicts(reactions: Sequence[FluxWireReaction]) -> List[Dict[str, Any]]:
    rows = []
...
def _qg_isotope_activity_payload(reference_data: FluxWireData) -> Dict[str, Dict[str, Any]]:
    ...
    for nuclide in reference_data.nuclides:
        ...
        payload[nuclide.isotope] = {
            "activity_bq": activity_bq,
            "activity_unc_bq": activity_unc_uci * 3.7e4 if activity_unc_uci > 0.0 else 0.0,
            "activity_uci": activity_uci if activity_uci > 0.0 else activity_bq / 3.7e4 if activity_bq > 0.0 else 0.0,
            "activity_unc_uci": activity_unc_uci,
            "n_peaks": int(sum(1 for peak in qg_reference_peaks(reference_data) if str(peak.get("isotope")) == nuclide.isotope)),
            "peak_energies": [
...
def cd_ratio_rows(flux_wire_artifacts: Sequence[Dict[str, Any]], metadata: RAFMMetadata) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, float]]]:
    by_key = {normalize_pairing_key(item["sample_id"], metadata.pairing_aliases): item for item in flux_wire_artifacts}
    rows: List[Dict[str, Any]] = []
    ...
    for cd_key, artifact in sorted(by_key.items()):
        ...
        if bare is None:
            continue
        bare_total = sum(float(value.get("activity_bq", 0.0)) for value in bare.get("isotopes", {}).values())
        cd_total = sum(float(value.get("activity_bq", 0.0)) for value in artifact.get("isotopes", {}).values())
...
def run_qg_benchmark(
    ...
    for qg_path in qg_files:
        ...
        isotope_payload = aggregate_isotope_results(
            ...
            sample_mass_g=(flux_wire_mass_mg(sample_key, metadata) or 0.0) / 1000.0
    ...
    unfolding_results = run_flux_wire_unfolding(all_reactions, paths.prior_spectrum_path, tree["unfolding"])
    summary = {
        "overall_passed": True,
...
def compare_rafm_completion_results(
    ...
    compare_root = (output_root or (raw_root.parent / f"{raw_root.name}_vs_{qg_root.name}")).resolve()
    ...
    raw_by_key = {(str(row.get("sample_id")), str(row.get("reaction_id"))): row for row in raw_rows}
    qg_by_key = {(str(row.get("sample_id")), str(row.get("reaction_id"))): row for row in qg_rows}
    ...
    for key in matched_keys:
        ...
        rel_activity = (raw_activity - qg_activity) / qg_activity if qg_activity else None
    ...
    write_rows_csv(reaction_comparison_rows, compare_root / "reaction_rate_comparison.csv")
    ...
    for method in ("discrete", "gls", "gravel", "mlem"):
        ...
        if n == 0:
            continue
        rel_flux = np.abs(raw_flux[:n] - qg_flux[:n]) / np.maximum(np.abs(qg_flux[:n]), 1e-30)
    ...
    summary = {
        ...
        "median_abs_activity_rel_error": float(np.median(activity_errors)) if activity_errors else 0.0,
        "max_abs_activity_rel_error": float(np.max(activity_errors)) if activity_errors else 0.0,
        "median_abs_rate_rel_error": float(np.median(rate_errors)) if rate_errors else 0.0,
...
def adapt_unfold_result(
    ...
    measured_rates = np.array([reaction.reaction_rate for reaction in reactions], dtype=float)
...
def simplified_response_matrix(reactions: Sequence[FluxWireReaction], energy_edges: np.ndarray) -> np.ndarray:
    rows = [_make_response_row(reaction.reaction_id, energy_edges, len(energy_edges) - 1) for reaction in reactions]
...
def save_unfolding_artifacts(
    ...
    table_rows = []
    for reaction, measured, predicted in zip(result.reactions_used, result.measured_rates, result.predicted_rates):
        table_rows.append(
            {
                "reaction": reaction,
...
def run_flux_wire_unfolding(
    ...
    valid_reactions = [reaction for reaction in reactions if reaction.reaction_rate > 0 and "Unknown(" not in reaction.reaction_id]
    if not valid_reactions:
        return {}

    discrete = unfold_discrete_bins(valid_reactions, n_bins=10)
    discrete_response = simplified_response_matrix(valid_reactions, discrete.energy_bounds_eV)
    ...
    iterative_results: Dict[str, UnfoldingResult] = {"DISCRETE": discrete_result, "GLS": gls_result}
    for method in ("GRAVEL", "MLEM"):
        ...
        for reaction in valid_reactions:
            unfolder.add_reaction(
                ...
                uncertainty_Bq=max(reaction.reaction_rate_unc, 0.05 * reaction.reaction_rate),
            )
    ...
    method_overlay_plot(iterative_results, output_root.parent / "plots" / "unfolding" / "method_overlay.png")
...
def run_rafm_validation(
    ...
    if generic_targeted_counting_method is not None:
        metadata.config["generic_targeted_counting_method"] = str(generic_targeted_counting_method)
    tree = ensure_results_tree(paths.results_root)
    ...
    pairs, unmatched_raw, unmatched_qg = pair_input_files(discovered["raw"], discovered["qg"], metadata.pairing_aliases)
    if max_spectra is not None:
    ...
    if background_spectrum is None:
        raise ValueError(f"Failed to load background spectrum from {paths.background_path}")
    ...
    for raw_path, qg_path, sample_key in pairs:
        ...
        if relative_parent == "flux_wires":
            artifact = analyze_flux_wire_sample(raw_path, metadata, paths, tree, background_spectrum, qg_path, sample_key)
            flux_wire_artifacts.append(artifact)
            reaction_rows.extend(artifact.get("reactions", []))
        else:
            artifact = analyze_generic_sample(raw_path, metadata, paths, tree, gamma_library, half_lives, background_spectrum, qg_path)
    ...
    write_rows_csv(fluxforge_consistency_rows, tree["tables"] / "fluxforge_line_consistency.csv")
    ...
    write_rows_csv(flux_wire_count_rows, tree["tables"] / "flux_wire_count_disagreement.csv")
    write_flux_wire_count_disagreement_summary(
        flux_wire_count_rows,
        tree["tables"] / "flux_wire_count_disagreement_summary.md",
    )
    write_rows_csv(reaction_rows, tree["tables"] / "flux_wire_reaction_rates.csv")
    write_rows_csv([{"raw_file": str(path)} for path in unmatched_raw], tree["tables"] / "unmatched_raw.csv")
    write_rows_csv([{"qg_file": str(path)} for path in unmatched_qg], tree["tables"] / "unmatched_qg.csv")
    ...
    summary_comparisons = [
        ...
        for row in isotope_rows if row.get("matched") and row.get("raw_activity_bq") is not None
    ]
    ...
    failing_samples = [artifact["sample_id"] for artifact in artifacts if not artifact["validation"].get("passed", True)]
    line_diagnostic_buckets: Dict[str, int] = {}
    for row in line_rows:
        bucket = row.get("diagnostic_bucket")
        if not bucket:
            continue
        line_diagnostic_buckets[str(bucket)] = line_diagnostic_buckets.get(str(bucket), 0) + 1
    summary = {
        ...
        "qg_internal_consistency_flags": int(sum(1 for row in qg_consistency_rows if row.get("flag_internal_inconsistency"))),
        "fluxforge_line_consistency_flags": int(sum(1 for row in fluxforge_consistency_rows if row.get("flag_line_inconsistency"))),
        "measurement_qc_flags": int(sum(1 for row in measurement_qc_rows if row.get("flag_review"))),
        "results_root": str(paths.results_root),
        "unfolding_methods": sorted(unfolding_results.keys()),
    }
    ...
    if enforce_thresholds and failing_samples:
        raise RuntimeError(
            "RAFM validation thresholds were violated for: " + ", ".join(failing_samples)
        )
