...
class ExperimentalDatabase:
    ...
    def get_experiments_by_energy_range(
        self, E_min: float, E_max: float
    ) -> List[Experiment]:
        """Get experiments within energy range."""
        return [e for e in self.experiments if E_min <= e.energy_MeV <= E_max]
    ...
    def from_json(cls, filepath: Path) -> "ExperimentalDatabase":
        ...
        db = cls(name=data.get("name", "unknown"), metadata=data.get("metadata", {}))
...
class SensitivityMatrix:
    ...
    def to_dataframe(self):
        ...
        try:
            ...
            return pd.DataFrame(
                self.matrix, index=self.experiment_names, columns=self.parameter_names
            )
        except ImportError:
            return None
...
class GMAWorkflow:
    ...
    def load_experiments(
        self, source: Union[str, Path, ExperimentalDatabase]
    ) -> "GMAWorkflow":
        """
    ...
    def calculate_sensitivities(
        ...
        if matrix is not None:
            ...
            param_names = (
                self.prior.parameter_names
                if self.prior
                else [f"p_{i}" for i in range(n_param)]
            )
    ...
    def export_results(self, filepath: Path, format: str = "json") -> None:
        ...
        if format == "json":
            data = {
                ...
                "created": datetime.now().isoformat(),
                "parameters": {
                    name: {"value": float(val), "uncertainty": float(unc)}
