...
class SpectrumExporter:
    ...
    def to_iec(self, filepath: Union[str, Path]) -> None:
        ...
        xml.append(
            f"    <RadInstrumentManufacturerName>FluxForge</RadInstrumentManufacturerName>"
        )
        xml.append(
            f'    <RadInstrumentIdentifier>{self.metadata.detector_id or "UNKNOWN"}</RadInstrumentIdentifier>'
        )
        xml.append("  </RadInstrumentInformation>")
        ...
        if self.metadata.start_time:
            xml.append(
                f"    <StartDateTime>{self.metadata.start_time.isoformat()}</StartDateTime>"
            )

        xml.append(
            f"    <RealTimeDuration>PT{self.metadata.real_time:.1f}S</RealTimeDuration>"
        )

        xml.append("    <Spectrum>")
        xml.append(
            f"      <LiveTimeDuration>PT{self.metadata.live_time:.1f}S</LiveTimeDuration>"
        )
    ...
    def to_json(self, filepath: Union[str, Path]) -> None:
        ...
        data = {
            ...
            "metadata": {
                ...
                "start_time": (
                    self.metadata.start_time.isoformat()
                    if self.metadata.start_time
                    else None
                ),
    ...
    def to_hdf5(
        self, filepath: Union[str, Path], dataset_name: str = "spectrum"
    ) -> None:
        """
