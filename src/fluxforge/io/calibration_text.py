...
def read_legacy_spectrum_calibration(path: Union[str, Path]) -> PolynomialCalibrationFile:
    """
    Read legacy_spectrum-style calibration file.
    """
    path = Path(path)
    lines = [
        line.strip()
...
def legacy_spectrum_to_calibration(data: PolynomialCalibrationFile) -> Calibration:
    """
    Convert legacy_spectrum calibration file contents to FluxForge Calibration.
    """
    # legacy_spectrum stores coefficients in descending order.
