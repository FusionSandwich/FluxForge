...
def _get_text(
    parent: ET.Element, tag: str, namespaces: Dict[str, str], default: str = ""
) -> str:
...
def _parse_numeric_values(text: str) -> Optional[List[float]]:
    """Parse whitespace/comma separated numeric values from text."""
    try:
...
def _parse_calibration(
    cal_elem: ET.Element, namespaces: Dict[str, str]
) -> Tuple[float, ...]:
...
def _parse_measurement(
    ...
    if det_elem is not None:
        ...
        measurement.detector_description = _get_text(
            det_elem, "DetectorDescription", namespaces
        )
