...
FILE_DATE_RE = re.compile(
    r"^\s*File:\s*(\S+)\s+Date:\s*(.+)$", re.IGNORECASE | re.MULTILINE
)
...
LT_RT_DT_RE = re.compile(
    r"LT:\s*([\d.,]+)\s*RT:\s*([\d.,]+)\s*DT:\s*([\d.,]+)\s*%?", re.IGNORECASE
)
...
EFF_COEF_RE = re.compile(
    r"^\s*C([1-4]):\s*([+-]?\d+\.?\d*(?:[eE][+-]?\d+)?)", re.MULTILINE
)

# Geometry factor
GEOM_A_RE = re.compile(
    r"Geometry Factor \(A\):\s*([+-]?\d+\.?\d*(?:[eE][+-]?\d+)?)", re.IGNORECASE
)
...
DEAD_LAYER_RE = re.compile(
    r"Detector Dead Layer \(DL\):\s*([\d.,]+)\s*um", re.IGNORECASE
)
DET_ANGLE_RE = re.compile(
    r"Det\. Incident Angle \(AI\):\s*([\d.,]+)\s*deg", re.IGNORECASE
)
...
class EfficiencyCalibration:
    ...
    def efficiency(
        self, energy_keV: Union[float, np.ndarray]
