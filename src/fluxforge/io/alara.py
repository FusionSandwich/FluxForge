...
class ALARASettings:
    ...
    irradiation_schedule: List[Tuple[str, str]] = field(
        default_factory=lambda: [("1h", "flux_1")]
    )
...
def build_zone_stats(
    ...
    stats = filtered.groupby(["run_lbl", "nuclide", "time"], as_index=False)[
        "value"
...
def build_isotope_table(stats: "pd.DataFrame") -> "pd.DataFrame":
    ...
    time_order = []
    if not stats.empty:
        ordered = stats[["time", "time_label"]].drop_duplicates().sort_values("time")
    ...
    for col in ["mean", "sem", "rel_unc"]:
        pivot = stats.pivot_table(
            index="nuclide", columns="time_label", values=col, aggfunc="first"
        )
