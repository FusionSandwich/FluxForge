...
def parse_mcnp_input(input_path: str) -> Dict[str, Any]:
    ...
    for raw_line in lines:
        ...
        if current_mat and (
            raw_line.startswith("     ") or raw_line.strip().endswith("&")
        ):
...
def read_mcnp_spectrum_csv(
    ...
    *,
    ...
    if n_cols == 4:
        ...
        uncertainty = np.sqrt(np.sum(abs_unc**2, axis=1)) / (
            n_zones * np.maximum(flux, 1e-30)
        )
    else:
...
class MCTALFile:
    ...
    def get_tally(self, tally_id: int) -> MCTALTally:
        ...
        if tally_id not in self.tallies:
            raise KeyError(
                f"Tally {tally_id} not found. Available: {list(self.tallies.keys())}"
            )
...
def _parse_tally_block(lines: List[str], start_idx: int, tally: MCTALTally) -> int:
    ...
    while i < n:
        ...
        if line.startswith("tfc"):
            ...
            # Skip the tfc data
            while i < n:
                if lines[i].strip().startswith("tally") or lines[i].strip().startswith(
                    "kcode"
                ):
                    break
                i += 1
