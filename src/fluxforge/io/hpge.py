...
FILE_DATE_RE = re.compile(
    r"^\s*File:\s*(.+?)\s+Date:\s*(.+)$", re.IGNORECASE | re.MULTILINE
)
    ...
    try:
        ...
        if trailer_start + 12 <= len(data):
            try:
                ...
                cal_offset = struct.unpack(
                    "<f", data[trailer_start : trailer_start + 4]
                )[0]
                cal_gain = struct.unpack(
                    "<f", data[trailer_start + 4 : trailer_start + 8]
                )[0]
                cal_quad = struct.unpack(
                    "<f", data[trailer_start + 8 : trailer_start + 12]
                )[0]
...
def chn_to_hpge_report(
    chn_spectrum: CHNSpectrum, report_id: Optional[str] = None
) -> HPGeReport:
    """
    ...
    return HPGeReport(
        report_id=(
            report_id or Path(source_file).stem if source_file else "chn_spectrum"
        ),
