...
from fluxforge.physics.neutron_corrections import calculate_cd_ratio, extract_thermal_epithermal_components
from fluxforge.data.k0_library import get_k0_library_record
...
def _coerce_segments(payload: dict[str, Any]) -> list[IrradiationSegment]:
    ...
    duration_s = float(irradiation.get("duration_s", payload.get("duration_s", 0.0)) or 0.0)
    return [IrradiationSegment(duration_s=duration_s, relative_power=1.0)] if duration_s > 0.0 else []
...
def equivalent_irradiation_duration_s(segments: Sequence[IrradiationSegment]) -> float:
    ...
    return float(sum(segment.duration_s * segment.relative_power for segment in segments))


def _measurement_from_row(row: dict[str, Any], prefix: str = "") -> GammaLineMeasurement:
    ...
    return GammaLineMeasurement(
        ...
        gamma_intensity=float(
            row.get(
                key("gamma_intensity"),
                row.get(key("emission_probability"), row.get("gamma_intensity", row.get("emission_probability", 0.0))),
            )
        ),
        half_life_s=float(row.get(key("half_life_s"), row.get("half_life_s", 0.0))),
        cooling_time_s=float(row.get(key("cooling_time_s"), row.get(key("decay_time_s"), row.get("cooling_time_s", 0.0))) or 0.0),
        dead_time_fraction=float(row.get(key("dead_time_fraction"), row.get("dead_time_fraction", 0.0)) or 0.0),
    )


def _reaction_rate_from_measurement(measurement: GammaLineMeasurement, segments: Sequence[IrradiationSegment]) -> tuple[float, float, float, float]:
    activity_eoi_bq = measurement.activity_at_reference()
    activity_unc_bq = float(activity_eoi_bq / math.sqrt(max(measurement.net_counts, 1.0)))
    buildup_factor = irradiation_buildup_factor(segments, measurement.half_life_s)
    if buildup_factor <= 0.0:
        raise ValueError("Irradiation buildup factor must be positive for ASTM E262 analysis.")
...
def _true_thermal_from_equivalent(phi0_cm2_s: float, neutron_temperature_K: float) -> float:
    if phi0_cm2_s <= 0.0:
        return 0.0
...
def _analyze_radiometric_measurement(
    ...
    *,
    ...
    reaction_rate_s, reaction_rate_unc_s, activity_eoi_bq, activity_unc_bq = _reaction_rate_from_measurement(measurement, segments)

    sigma_0_barn = float(row.get("sigma_0_barn", row.get("thermal_cross_section_barn", 0.0)))
    sigma_0_unc_barn = float(row.get("sigma_0_unc_barn", row.get("thermal_cross_section_unc_barn", 0.0)) or 0.0)
    ...
    if sigma_0_barn <= 0.0:
        ...
        iso_fmt = iso_raw[:-2] + "-" + iso_raw[-2:] if len(iso_raw) > 2 and iso_raw[-2:].isdigit() and "-" not in iso_raw else iso_raw
    ...
    thermal_self_shielding_factor = float(row.get("thermal_self_shielding_factor", row.get("G_th", 1.0)) or 1.0)

    denominator = sigma_0_barn * BARN_TO_CM2 * westcott_g * thermal_self_shielding_factor
    if denominator <= 0.0:
        raise ValueError("ASTM E262 radiometric mode requires positive sigma_0_barn, Westcott g, and thermal self-shielding factor.")

    thermal_rate_s = reaction_rate_s
    thermal_rate_unc_s = reaction_rate_unc_s
    cadmium_payload: dict[str, Any] = {}

    has_cd_pair = any(key in row for key in ("cd_net_counts", "cd_live_time_s", "cd_efficiency", "cd_gamma_intensity"))
    if has_cd_pair:
        cd_measurement = _measurement_from_row(row, prefix="cd_")
        cd_reaction_rate_s, cd_reaction_rate_unc_s, cd_activity_bq, cd_activity_unc_bq = _reaction_rate_from_measurement(cd_measurement, segments)
        cd_transmission_factor = float(row.get("cd_transmission_factor", row.get("cadmium_transmission_factor", 1.0)) or 1.0)
        thermal_rate_s = max(reaction_rate_s - cd_transmission_factor * cd_reaction_rate_s, 0.0)
        thermal_rate_unc_s = float(
            math.sqrt(
                reaction_rate_unc_s * reaction_rate_unc_s
                + (cd_transmission_factor * cd_reaction_rate_unc_s) * (cd_transmission_factor * cd_reaction_rate_unc_s)
            )
        )
    ...
    phi0_eq_unc_cm2_s = float(phi0_eq_cm2_s * math.sqrt(rel_rate_unc * rel_rate_unc + rel_sigma_unc * rel_sigma_unc))

    neutron_temperature_K = float(row.get("neutron_temperature_K", default_neutron_temperature_K) or default_neutron_temperature_K)
    true_thermal_fluence_rate_cm2_s = _true_thermal_from_equivalent(phi0_eq_cm2_s, neutron_temperature_K)
    true_thermal_fluence_rate_unc_cm2_s = _true_thermal_from_equivalent(phi0_eq_unc_cm2_s, neutron_temperature_K)
    ...
    result = {
        ...
        "equivalent_2200ms_fluence_unc_cm2": float(phi0_eq_unc_cm2_s * fluence_duration_s),
        "true_thermal_fluence_rate_cm2_s": float(true_thermal_fluence_rate_cm2_s),
        "true_thermal_fluence_rate_unc_cm2_s": float(true_thermal_fluence_rate_unc_cm2_s),
        "true_thermal_fluence_cm2": float(true_thermal_fluence_rate_cm2_s * fluence_duration_s),
        "true_thermal_fluence_unc_cm2": float(true_thermal_fluence_rate_unc_cm2_s * fluence_duration_s),
...
def _analyze_standard_comparison_measurement(
    ...
    *,
    ...
    unknown_rate_s, unknown_rate_unc_s, unknown_activity_bq, unknown_activity_unc_bq = _reaction_rate_from_measurement(unknown_measurement, segments)
    standard_rate_s, standard_rate_unc_s, standard_activity_bq, standard_activity_unc_bq = _reaction_rate_from_measurement(standard_measurement, segments)

    known_phi0_cm2_s = float(row.get("known_reference_fluence_rate_cm2_s", row.get("known_fluence_rate_cm2_s", 0.0)))
    known_phi0_unc_cm2_s = float(row.get("known_reference_fluence_rate_unc_cm2_s", row.get("known_fluence_rate_unc_cm2_s", 0.0)) or 0.0)
    if known_phi0_cm2_s <= 0.0:
        raise ValueError("ASTM E262 standard-comparison mode requires known_reference_fluence_rate_cm2_s > 0.")
    if standard_rate_s <= 0.0:
        raise ValueError("ASTM E262 standard-comparison mode requires a positive standard reaction rate.")

    spectral_correction_factor = float(row.get("spectral_correction_factor", 1.0) or 1.0)
    if spectral_correction_factor <= 0.0:
        raise ValueError("ASTM E262 standard-comparison mode requires spectral_correction_factor > 0.")

    phi0_eq_cm2_s = float(known_phi0_cm2_s * (unknown_rate_s / standard_rate_s) * spectral_correction_factor)
    rel_unc = math.sqrt(
...
def analyze_astm_e262_plan(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not segments:
        raise ValueError("ASTM E262 workflow requires a positive irradiation duration or segment list.")
    ...
    fluence_duration_s = float((plan.get("irradiation") or {}).get("fluence_duration_s", equivalent_irradiation_duration_s(segments)))
    default_neutron_temperature_K = float(plan.get("neutron_temperature_K", T0_K) or T0_K)
    ...
    for index, row in enumerate(measurement_rows, start=1):
        ...
        result = {
            ...
            "reaction_id": str(row.get("reaction_id") or row.get("monitor_id") or row.get("product_isotope") or f"reaction_{index}"),
        ...
        result.update(calc)
        if "activity_eoi_Bq" in result and "activity_eoi_unc_Bq" in result and row.get("half_life_s") is not None:
            result.update(
                activation_study_metrics(
                    ...
                    sample_mass_g=float(row.get("sample_mass_g")) if row.get("sample_mass_g") is not None else None,
                )
            )
    ...
    summary = {
        ...
        "fluence_duration_s": float(fluence_duration_s),
        "max_equivalent_2200ms_fluence_cm2": float(max((row["equivalent_2200ms_fluence_cm2"] for row in results), default=0.0)),
        "max_equivalent_2200ms_fluence_rate_cm2_s": float(max((row["equivalent_2200ms_fluence_rate_cm2_s"] for row in results), default=0.0)),
    ...
    return {
        ...
        "inputs": {
            "irradiation": {
                "segments": [
                    {"duration_s": float(segment.duration_s), "relative_power": float(segment.relative_power)} for segment in segments
                ],
                "fluence_duration_s": float(fluence_duration_s),
            },
