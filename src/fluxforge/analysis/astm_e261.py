...
def target_atom_count(
	*,
	...
	return float((effective_mass / atomic_mass_g_mol) * AVOGADRO * atoms_per_formula_unit)
...
def _coerce_segments(payload: dict[str, Any]) -> list[IrradiationSegment]:
	...
	if raw_segments:
		return [IrradiationSegment(duration_s=float(row.get("duration_s", 0.0)), relative_power=float(row.get("relative_power", 1.0))) for row in raw_segments]
...
def _build_measurement(row: dict[str, Any]) -> GammaLineMeasurement:
	return GammaLineMeasurement(
		...
		gamma_intensity=float(row.get("gamma_intensity", row.get("emission_probability", 0.0))),
		half_life_s=float(row.get("half_life_s", 0.0)),
		cooling_time_s=float(row.get("cooling_time_s", row.get("decay_time_s", 0.0)) or 0.0),
...
def analyze_astm_e261_plan(plan: dict[str, Any]) -> dict[str, Any]:
	...
	if not segments:
		raise ValueError("ASTM E261 workflow requires a positive irradiation duration or segment list.")
	...
	for index, row in enumerate(measurement_rows, start=1):
		...
		if buildup_factor <= 0.0:
			raise ValueError("Irradiation buildup factor must be positive for ASTM E261 analysis.")
		...
		effective_cross_section_barn = float(row.get("effective_cross_section_barn", 0.0))
		effective_cross_section_unc_barn = float(row.get("effective_cross_section_unc_barn", 0.0) or 0.0)
		correction_factor = (
		...
		denominator = target_atoms * effective_cross_section_barn * BARN_TO_CM2 * correction_factor
		if denominator <= 0.0:
			raise ValueError("ASTM E261 workflow requires positive target atoms, cross section, and correction factor.")
		...
		rel_sigma_unc = _relative_uncertainty(effective_cross_section_barn, effective_cross_section_unc_barn)
		fluence_rate_unc_cm2_s = float(fluence_rate_cm2_s * math.sqrt(rel_rate_unc * rel_rate_unc + rel_sigma_unc * rel_sigma_unc))
	...
	summary = {
		...
		"fluence_duration_s": float(fluence_duration_s),
		"max_fluence_cm2": float(max((row["fluence_cm2"] for row in results), default=0.0)),
		"max_fluence_rate_cm2_s": float(max((row["fluence_rate_cm2_s"] for row in results), default=0.0)),
