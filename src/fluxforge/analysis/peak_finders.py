...
def estimate_background_linear(
    spectrum: np.ndarray, window_size: int = 50
) -> np.ndarray:
    """
...
def savitzky_golay_smooth(
    data: np.ndarray, window_size: int = 11, order: int = 3
) -> np.ndarray:
    """
...
class WindowPeakFinder:
    ...
    def _window_stats(self, data: np.ndarray, index: int) -> Tuple[float, float]:
        ...
        for i in range(self.n_outer, n - self.n_outer):
            ...
            if value > local_threshold:
                ...
                if (
                    ...
                    and spectrum[i - 1] >= local_threshold
                ):
                    ...
                    peaks.append(
                        PeakInfo(index=i, value=float(value), significance=significance)
                    )
...
class ScipyPeakFinder:
    ...
    def find(self, spectrum: np.ndarray) -> List[PeakInfo]:
        ...
        # Find peaks
        kwargs = {"height": (threshold, None), "distance": self.distance}
        ...
        for i, idx in enumerate(indices):
            ...
            height_above = (
                spectrum[idx] - threshold[idx] if idx < len(threshold) else 0.0
            )
            noise = (
                np.sqrt(np.maximum(threshold[idx], 1.0))
...
class DirectScipyPeakFinder:
    ...
    def find(self, spectrum: np.ndarray) -> List[PeakInfo]:
        ...
        return [
            PeakInfo(index=int(idx), value=float(spectrum[int(idx)])) for idx in indices
        ]
...
def refine_peak_centroids(
    spectrum: np.ndarray, peaks: List[PeakInfo], width: int = 3
) -> List[PeakInfo]:
    """
...
def merge_nearby_peaks(peaks: List[PeakInfo], min_distance: int = 3) -> List[PeakInfo]:
    """
...
def weighted_moving_average(data: np.ndarray, window_size: int = 5) -> np.ndarray:
    """
    ...
    weights = list(range(1, math.ceil(window_size / 2) + 1)) + list(
        range(math.floor(window_size / 2), 0, -1)
    )
        ...
        for i in range(2, n - 2):
            if d2[i] < threshold[i] and spectrum[i] >= self.min_counts:
                ...
                if d2[i] < d2[i - 1] and d2[i] < d2[i + 1]:
                    ...
                    peaks.append(
                        PeakInfo(
                            index=i, value=float(spectrum[i]), significance=significance
                        )
                    )
...
def get_peak_finder(method: str = "simple", **kwargs):
    ...
    **kwargs
    ...
    if method not in PEAK_FINDER_METHODS:
        raise ValueError(
            f"Unknown method '{method}'. Available: {list(PEAK_FINDER_METHODS.keys())}"
        )
