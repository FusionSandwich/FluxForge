...
def gaussian(
    x: np.ndarray, amplitude: float, centroid: float, sigma: float
) -> np.ndarray:
    """Gaussian function."""
...
def gauss_dbl_exp(
    ...
    if ltail_ratio > 0:
        ...
        if np.any(mask):
            ltail_func[mask] = (
                amplitude * ltail_ratio * np.exp(ltail_slope * (x[mask] - centroid))
            )
            ltail_func[mask] *= -np.expm1(
                ltail_cutoff * alpha * ((x[mask] - centroid) ** 2)
            )
    ...
    if rtail_ratio > 0:
        ...
        if np.any(mask):
            rtail_func[mask] = (
                amplitude * rtail_ratio * np.exp(rtail_slope * (x[mask] - centroid))
            )
            rtail_func[mask] *= -np.expm1(
                rtail_cutoff * alpha * ((x[mask] - centroid) ** 2)
            )
...
def gauss_with_erf(
    x: np.ndarray, amp_gauss: float, amp_erf: float, centroid: float, sigma: float
) -> np.ndarray:
    """
...
def poisson_neg_log_likelihood(y_model: np.ndarray, y_data: np.ndarray) -> float:
    """
    ...
    if model == "gaussian":
        ...
        if background == "linear":
            ...
            def model_func(params):
                ...
                return (
                    gauss_dbl_exp(x_fit, amp, cent, sig, lr, ls, lc, rr, rs, rc)
                    + m * x_fit
                    + b
                )
...
class ExpGaussPeak:
    ...
    def __repr__(self) -> str:
        return (
            f"ExpGaussPeak(centroid={self.centroid:.2f}, "
...
def _snip_background(
    counts: np.ndarray, iterations: Union[int, List[int]] = 20
) -> np.ndarray:
    """
...
def subtract_background(
    counts: np.ndarray, background: np.ndarray, minimum: float = 0.0
) -> np.ndarray:
    """
...
def window_peak_finder(
    ...
    for i in range(outer_window, n - outer_window):
        ...
        window_vals = np.concatenate(
            [counts[left_start:left_end], counts[right_start:right_end]]
        )
...
def chunked_peak_finder(
    ...
    for chunk_idx in range(n_chunks):
        ...
        peaks, properties = signal.find_peaks(
            chunk_counts, height=local_threshold, distance=min_distance
        )
...
def auto_find_peaks(
    ...
    if finder_method == "window":
        ...
        result = window_peak_finder(
            channels, counts, threshold, min_distance=min_distance
        )
        return [(ch, sig) for ch, _, sig in result]

    elif finder_method == "chunked":
        return chunked_peak_finder(
            channels, counts, threshold, min_distance=min_distance
        )
    ...
    # Find local maxima
    peaks, properties = signal.find_peaks(
        significance, height=threshold, distance=min_distance
    )
    ...
    if share_sigma:
        ...
        def model(x_vals: np.ndarray, *params: float) -> np.ndarray:
            ...
            total = (
                ...
                if background_model == "linear"
                else np.full_like(x_vals, params[-1], dtype=float)
            )
            for i in range(len(peak_channels)):
                total = total + gaussian(
                    x_vals, params[2 * i], params[2 * i + 1], sigma
                )
            return total
        ...
        def model(x_vals: np.ndarray, *params: float) -> np.ndarray:
            ...
            total = (
                params[bg_offset] * x_vals + params[bg_offset + 1]
                if background_model == "linear"
                else np.full_like(x_vals, params[bg_offset], dtype=float)
            )
            for i in range(len(peak_channels)):
                offset = 3 * i
                total = total + gaussian(
                    x_vals, params[offset], params[offset + 1], params[offset + 2]
                )
            return total
        ...
        for amp_guess, centroid_guess in zip(amplitude_guesses, refined_centroids):
            ...
            bounds_upper.extend(
                [
                    np.inf,
                    centroid_guess + max(2.0, fit_width / 2.0),
                    float(max(fit_width, 2.0 * sigma_guess)),
                ]
            )
        if background_model == "linear":
            p0.extend([bg_slope, bg_intercept])
...
def hypermet_with_linear_bg(
    ...
    return (
        hypermet_function(
            x, amplitude, centroid, sigma, tail_left, tail_left_frac, step_height
        )
        + bg_slope * x
        + bg_intercept
    )
