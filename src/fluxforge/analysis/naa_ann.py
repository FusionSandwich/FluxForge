...
class SpectralAugmentor:
    ...
    def _generate_one_synthetic(self, spectrum: np.ndarray) -> np.ndarray:
        ...
        noise = (
            self._rng.normal(0, 1, n_channels)
            * np.sqrt(np.maximum(synth, 1))
        ...
        intensity_factor = 1.0 + self._rng.uniform(
            -self.config.intensity_variation, self.config.intensity_variation
        )
        synth *= intensity_factor

        # 3. Apply energy shift (channel shift)
        shift = self._rng.uniform(
            -self.config.energy_shift_max, self.config.energy_shift_max
        )
        if abs(shift) > 0.1:
        ...
        if self.config.peak_width_variation > 0:
            ...
            if width_change > 0.01:
                ...
                kernel = np.exp(
                    -np.arange(-kernel_width, kernel_width + 1) ** 2
                    / (2 * (width_change * 2 + 0.5) ** 2)
                )
    ...
    def augment_dataset(
        ...
        all_params = (
            list(experimental_params) if experimental_params is not None else None
        )
        ...
        for i, spectrum in enumerate(spectra):
            ...
            for _ in synthetic:
                ...
                if all_params is not None:
                    ...
                    varied_params *= 1 + self._rng.uniform(
                        -0.01, 0.01, len(varied_params)
                    )
...
class NAAANNModel:
    ...
    def build(self):
        ...
        loss_weights_dict = {
            name: loss_weights_list[i] for i, name in enumerate(output_names)
        }
    ...
    def train(
        ...
        y_train_dict = {
            name: y_train_proc[:, i] for i, name in enumerate(self._output_names)
        }
        ...
        if X_val is not None and y_val is not None:
            ...
            y_val_dict = {
                name: y_val_proc[:, i] for i, name in enumerate(self._output_names)
            }
        ...
        if save_best:
            callbacks.append(
                ModelCheckpoint(
                    save_best, monitor=monitor, mode="min", save_best_only=True
                )
            )
...
def _load_naa_ann4e_counts(
    ...
    with zipfile.ZipFile(zip_path) as zf:
        data_files = [
            ...
            and (name.startswith("4e_data_") or name.startswith("4e_synth_"))
        ]
...
def prepare_naa_ann4e_dataset(
    ...
    denom = np.where(
        dataset.label_max - dataset.label_min > 0,
        dataset.label_max - dataset.label_min,
        1.0,
    )
