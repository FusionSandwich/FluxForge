...
def find_peak_candidates(
    counts: NDArray, threshold_sigma: float = 5.0, min_counts: float = 100.0
    ...
    peaks, properties = find_peaks(
        significance, height=threshold_sigma, distance=3, prominence=2
    )
...
def refine_calibration(
    counts: NDArray, current_coeffs: NDArray, isotopes: List[str], degree: int = 2
) -> AutoCalibrationResult:
    """
...
if __name__ == "__main__":
    ...
    result = auto_calibrate(
        counts, isotopes=["Co-60", "Cs-137"], initial_gain=0.5, initial_offset=0.0
    )
