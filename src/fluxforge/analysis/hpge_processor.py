...
def identify_isotope_from_energy(
    energy: float, tolerance: float = 1.0
    ...
    def analyze(
        ...
        if fit_all_peaks or not known_isotopes:
            ...
            for peak_ch, significance in peaks:
                ...
                if any(
                    abs(gl.energy - peak_energy) < self.energy_tolerance
        ...
        return HPGeAnalysisResult(
            ...
            metadata={
                ...
                "background_source": (
                    background_spectrum.spectrum_id if background_spectrum else None
                ),
            },
        )
    ...
    def compare_with_alara(
        ...
        for isotope in all_isotopes:
            ...
            comparison[isotope] = {
                ...
                "ratio_unc": ratio_unc,
                "difference_sigma": (
                    abs(meas_val - alara_val) / meas_unc if meas_unc > 0 else 0
                ),
            }
