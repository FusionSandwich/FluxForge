...
def electronic_stopping_ziegler(
    energy_MeV: float, projectile: Projectile, target_Z: int, target_A: float
) -> float:
    """
...
def total_stopping_power(
    energy_MeV: float, projectile: Projectile, material: Material
) -> float:
    """
...
def bohr_straggling(
    thickness_mg_cm2: float, projectile_Z: int, target_Z: int, target_A: float
) -> float:
    """
...
def calculate_straggling(
    ...
    return StragglResult(
        mean_energy_MeV=E_final, sigma_MeV=sigma, fwhm_MeV=2.355 * sigma
    )
