...
class CorrectionType(Enum):
    ...
    SATURATION_WITH_SAMPLING_DECAY = (
        "saturation_with_sampling_decay"  # With sampling decay
    )
    ...
    if apply_burnup:
        if burnup_model is not None:
            ...
            if estimated_depletion > burnup_threshold:
                burnup_corr = (1.0 - math.exp(-estimated_depletion)) / max(
                    estimated_depletion, 1e-30
                )
