...
class NuclideData:
    ...
    def get_gamma_energies(self, min_intensity: float = 0.0) -> np.ndarray:
        ...
        return np.array(
            [g.energy_eV for g in self.gamma_lines if g.intensity >= min_intensity]
        )

    def get_gamma_intensities(self, min_intensity: float = 0.0) -> np.ndarray:
        """Get array of gamma intensities above threshold."""
        return np.array(
            [g.intensity for g in self.gamma_lines if g.intensity >= min_intensity]
        )
...
class NuclideDatabase:
    ...
    def get_gamma_lines(
        self, name: str, min_intensity: float = 0.0
    ) -> List[GammaLineData]:
    ...
    def load_json(self, filepath: str) -> None:
        ...
        for name, nuc_data in data.items():
            ...
            if "gamma" in nuc_data and "lines" in nuc_data["gamma"]:
                ...
                for e, i, eu, iu in zip(
                    energies, intensities, energy_uncs, intensity_uncs
                ):
                    gamma_lines.append(
                        GammaLineData(
                            energy_eV=e, intensity=i, energy_unc_eV=eu, intensity_unc=iu
                        )
                    )
    ...
    def load_builtin(self) -> None:
        ...
        common_nuclides = [
            ...
            NuclideData(
                ...
                gamma_lines=[GammaLineData(661657.0, 0.8510)],
            ),
            NuclideData(
                name="Eu152",
                zai=631520,
            ...
            NuclideData(
                ...
                gamma_lines=[GammaLineData(411802.0, 0.9562)],
            ),
            NuclideData(
                name="In116m",
                zai=491161,
