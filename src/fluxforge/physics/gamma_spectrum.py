...
def get_decay_lines(
    nuclide: str, emission_types: Optional[List[str]] = None
) -> List[DecayLine]:
    """
    ...
    for E, I, etype in lines:
        if etype in emission_types:
            result.append(
                DecayLine(
                    nuclide=nuclide, energy_keV=E, intensity=I, emission_type=etype
                )
            )
...
def generate_spectrum(
    ...
    for nuclide, activity in inventory.activities.items():
        ...
        nuclide_spectrum = bin_decay_lines(lines, bins, activity, energy_conservation)
...
def identify_nuclides(
    peak_energies: List[float], tolerance_keV: float = 1.0, min_intensity: float = 0.01
) -> List[List[PeakMatch]]:
    """
...
def suggest_nuclides(
    peak_energies: List[float], tolerance_keV: float = 1.0, top_n: int = 10
...
def atoms_to_activity(atoms: float, half_life_s: float) -> float:
...
def decay_activity(activity_0: float, half_life_s: float, time_s: float) -> float:
...
if __name__ == "__main__":
    ...
    print(
        f"Energy bins: {bins.n_bins} bins, {bins.edges[0]:.0f}-{bins.edges[-1]:.0f} keV"
    )
    ...
    for idx in peak_indices:
        print(
            f"  {spectrum.energies[idx]:.1f} keV: {spectrum.intensities[idx]:.2e} photons/s"
        )
