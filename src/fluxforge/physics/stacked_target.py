...
class CrossSectionLibrary:
    ...
    def get_cross_section(
        cls, reaction: str, library: str = "irdff"
...
def _init_monitor_reactions():
    ...
    CrossSectionLibrary.register_reaction("Cu-63(p,n)Zn-63", E, sigma, "irdff")
    ...
    CrossSectionLibrary.register_reaction("Al-27(p,x)Na-22", E, sigma, "irdff")

    # Ti-nat(p,x)V-48 - common monitor
    E = np.array([5, 8, 10, 12, 15, 20, 25, 30, 40, 50])
    sigma = np.array([5, 50, 200, 320, 380, 350, 280, 200, 100, 60])
    CrossSectionLibrary.register_reaction("Ti-nat(p,x)V-48", E, sigma, "irdff")
...
if __name__ == "__main__":
    ...
    stack = StackedTarget(projectile=Projectile.PROTON, beam_energy_MeV=30.0)
    ...
    for e_info in energies:
        foil = stack.foils[e_info.foil_index]
        print(
            f"  Foil {e_info.foil_index} ({foil.material.name}, {foil.thickness_um}μm):"
        )
        print(
            f"    E_in={e_info.energy_in_MeV:.2f} MeV, E_out={e_info.energy_out_MeV:.2f} MeV"
        )
        print(
            f"    E_mean={e_info.energy_mean_MeV:.2f} MeV, σ={e_info.straggling.sigma_MeV:.3f} MeV"
        )
    ...
    for rxn in ["Cu-63(p,n)Zn-63", "Al-27(p,x)Na-22", "Ti-nat(p,x)V-48"]:
        ...
        if xs:
            print(
                f"  {rxn}: {len(xs.energies_MeV)} points, max σ={max(xs.cross_sections_mb):.1f} mb"
            )
