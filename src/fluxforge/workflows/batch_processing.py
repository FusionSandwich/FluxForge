...
class BatchProcessingConfig:
    ...
    tier1_elements: List[str] = field(
        default_factory=lambda: ["W", "Cr", "Fe", "Ta", "Co", "Mn", "V", "Al", "Tb"]
    )
...
class BatchResult:
    ...
    def get_by_sample_time(
        self, sample: str, timepoint: str
        ...
        for path, result in self.results.items():
            if (
                ...
                and normalize_timepoint(result.timepoint) == tp_norm
            ):
...
def process_single_spectrum(
    ...
    if efficiency_model is not None:
        corrected = apply_efficiency_correction(
            spectrum.counts.astype(float), spectrum.energies, efficiency_model
        )
...
def process_batch(config: BatchProcessingConfig) -> BatchResult:
    ...
    spectra_files = sorted(
        glob(os.path.join(config.spectra_dir, config.spectrum_pattern))
    )

    if not spectra_files:
        raise ValueError(
            f"No spectra found in {config.spectra_dir} matching {config.spectrum_pattern}"
        )
