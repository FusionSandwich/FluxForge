...
class ActivationComparison:
    ...
    def __post_init__(self):
        ...
        if self.measured.activity_Bq > 0:
            ...
            rel_c = (
                self.calculated.activity_unc / self.calculated.activity_Bq
                if self.calculated.activity_Bq > 0
                else 0
            )
            rel_e = (
                self.measured.activity_unc / self.measured.activity_Bq
                if self.measured.activity_Bq > 0
                else 0
            )
    ...
    def summary(self) -> str:
        ...
        if self.activation_results:
            ...
            for ar in self.activation_results:
                lines.append(
                    f"  {ar.isotope}: {ar.activity_Bq:.4e} ± {ar.activity_unc:.4e} Bq"
                )
        ...
        if self.comparisons:
            ...
            for c in self.comparisons:
                ...
                lines.append(
                    f"  {c.isotope}: {c.c_over_e:.4f} ± {c.c_over_e_unc:.4f} [{status}]"
                )
...
"""
    ...
    def run_alara(
        ...
        if exe is None:
            return (
                False,
                f"ALARA executable not found: {self.alara_config.executable}",
                output_file,
            )
        ...
        try:
            ...
            return (
                False,
                f"ALARA timed out after {self.alara_config.timeout_s}s",
                output_file,
            )
    ...
    def run_pipeline(
        ...
        try:
            ...
            result.spectrum_file = self.generate_alara_flux(spectrum, energy_grid)
            ...
            # Step 3: Run ALARA
            success, output, result.alara_output = self.run_alara(result.alara_input)
            ...
            result.activation_results = self.parse_alara_output(result.alara_output)
            ...
            if measurements:
                for meas in measurements:
                    ...
                    for ar in result.activation_results:
                        ...
                        if (
                            ar.isotope.replace("-", "").lower()
                            == meas.isotope.replace("-", "").lower()
                        ):
                            calc = ar
                            break
