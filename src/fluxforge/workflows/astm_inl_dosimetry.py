...
class INLDosimetryWorkflow:
    ...
    def fwhm_at_energy(self, energy_keV: float, cal: DetectorCalibrationRecord) -> float:
        # Assuming FWHM = p[0] + p[1] * sqrt(E)
        if energy_keV <= 0: return cal.fwhm_poly[0]
        return cal.fwhm_poly[0] + cal.fwhm_poly[1] * math.sqrt(energy_keV)

    def channel_to_energy(self, channel: float, cal: DetectorCalibrationRecord) -> float:
        energy = 0.0
    ...
    def covell_linear(self, raw_counts: np.ndarray, roi_left: int, roi_right: int) -> Tuple[float, float]:
    ...
    def analyze_spectrum(self, raw_counts: np.ndarray, cal: DetectorCalibrationRecord) -> List[PeakAreaResult]:
    ...
    def compute_activity(self, peak: PeakAreaResult, cal: DetectorCalibrationRecord,
                        live_time: float, nuclide: str, record: DecayDataRecord) -> ActivityResult:
        if peak.net_area <= 0:
            return None
            
        eff = cal.efficiency_curve(peak.centroid_keV) if callable(cal.efficiency_curve) else cal.efficiency_curve.efficiency(peak.centroid_keV)
