...
print(
    f"  Posterior: c0={result.posterior_values[0]:.2f}±{result.posterior_uncertainties[0]:.2f}"
)
...
stack = StackedTarget(projectile=Projectile.PROTON, beam_energy_MeV=25.0)
...
for e in energies:
    ...
    print(
        f"  {foil.material.name:10} ({foil.thickness_um:4.0f}μm): "
...
for thickness in [10, 20, 50]:
    strag = calculate_straggling(
        E, Projectile.PROTON, STANDARD_MATERIALS["aluminum"], thickness
    )
    print(
        f"  After {thickness:2} mg/cm²: {strag.mean_energy_MeV:.2f}±{strag.sigma_MeV:.3f} MeV"
    )
...
response = [
    [0.3 * math.exp(-0.5 * (g - 1.8 * i) ** 2) for g in range(n_flux)]
...
if result.flux_uncertainty:
    avg_rel_unc = np.mean(
        [u / f for f, u in zip(result.flux, result.flux_uncertainty) if f > 0]
    )
...
for name, spec in [
    ("Smooth exponential", smooth_spec),
...
response = [
    [0.4 * math.exp(-0.3 * (g - 2 * i) ** 2) for g in range(n_flux)]
    for i in range(n_meas)
]
measurements = matmul(response, true_flux)
measurements = [
    max(m + np.random.normal(0, math.sqrt(max(m, 1))), 1) for m in measurements
]

for weight, label in [(0.0, "None"), (0.01, "Low"), (0.1, "High")]:
    result = mlem_with_covariance(
        response, measurements, smoothness_weight=weight, max_iters=200
    )
