...
def plot_chi2_comparison(results: dict, save_path: Path):
    ...
    bars = ax.bar(
        x, chi2_values, width, label="χ²/dof", color="steelblue", edgecolor="black"
    )
...
def main():
    ...
    try:
        ...
        predicted_rates = np.sum(
            response_matrix * flux_arr[np.newaxis, :] * dE[np.newaxis, :], axis=1
        )
        ...
        print(
            f"   χ²/dof = {gravel_sol.chi_squared:.4f}, iterations = {gravel_sol.iterations}"
        )
    ...
    try:
        ...
        print(
            f"   χ²/dof = {mlem_sol.chi_squared:.4f}, iterations = {mlem_sol.iterations}"
        )
    except Exception as e:
        print(f"   ERROR: {e}")
        import traceback
    ...
    if HAS_REF_UNFOLDING:
        ...
        try:
            ...
            diff = np.linalg.norm(ff_gravel - ref_gravel_norm) / np.linalg.norm(
                ref_gravel_norm
            )
            ...
            ax.loglog(
                energy_mid, ff_gravel, "b-", linewidth=2, label="FluxForge GRAVEL"
            )
            ax.loglog(
                energy_mid,
                ref_gravel_norm,
        ...
        try:
            ...
            diff = np.linalg.norm(ff_mlem - ref_mlem_norm) / np.linalg.norm(
                ref_mlem_norm
            )
            print(f"   Relative difference: {diff*100:.2f}%")
            ...
            ax.loglog(
                energy_mid, ref_mlem_norm, "r--", linewidth=2, label="Reference MLEM"
            )
    ...
    for method, result in results.items():
        print(
            f"  {method.upper():8s}: χ²/dof = {result.chi_squared:.4f}, "
