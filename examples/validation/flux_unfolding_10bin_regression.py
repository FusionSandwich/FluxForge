...
def save_spectrum_csv(
    path: Path, energy_bounds_eV: np.ndarray, flux: np.ndarray, flux_unc: np.ndarray
) -> None:
    with open(path, "w", encoding="utf-8", newline="") as f:
        ...
        for i in range(len(flux)):
            writer.writerow(
                [energy_bounds_eV[i], energy_bounds_eV[i + 1], flux[i], flux_unc[i]]
            )
...
def plot_comparison(
    ...
    def plot_panel(ax, discrete, gls, title: str) -> None:
        centers = np.sqrt(
            discrete.energy_bounds_eV[:-1] * discrete.energy_bounds_eV[1:]
        )
        ax.step(
            discrete.energy_bounds_eV[:-1],
            discrete.flux,
            where="post",
        ...
        ax.fill_between(
            gls_centers, gls.flux - gls.flux_unc, gls.flux + gls.flux_unc, alpha=0.2
        )
...
def run_unfolding_regression(
    proc_dir: Path, raw_dir: Path, model_path: Path
) -> UnfoldingResults:
...
def main() -> int:
    ...
    model_path = args.model_spectrum or (
        alara_root
        / "rafm_irradiation_ldrd"
        / "raw_gamma_spec"
        / "flux_wires"
        / "spectrum_vit_j.csv"
    )
    output_dir = args.output_dir or (
        repo_root / "artifacts" / "validation" / "flux_unfolding_10bin"
    )
