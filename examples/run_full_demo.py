...
DATA_DIR_ASC = Path(
    "/filespace/s/smandych/CAE/projects/ALARA/FluxForge/src/fluxforge/examples/flux_wire"
)
...
def run_spe_analysis():
    ...
    results = {"file": str(filepath), "calibration": spec.calibration, "peaks": []}

    if p1173_idx and p1332_idx:
        corrector = CoincidenceCorrector(MockEfficiencyCurve())

        for name, idx, energy_target in [
            ("1173 keV", p1173_idx, 1173.2),
            ("1332 keV", p1332_idx, 1332.5),
        ]:
