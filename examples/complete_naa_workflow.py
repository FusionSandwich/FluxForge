...
def demonstrate_k0_naa_workflow():
    ...
    print(
        f"  f (thermal/epithermal): {flux_params.f:.1f} ± {flux_params.f_uncertainty:.1f}"
    )
    print(
        f"  α (epithermal shape):   {flux_params.alpha:.3f} ± {flux_params.alpha_uncertainty:.3f}"
    )
    ...
    print(
        f"    Peak area: {au_measurement.net_peak_area:,.0f} ± {au_measurement.peak_area_unc:.0f} counts"
    )
    ...
    for meas in sample_measurements:
        try:
            ...
            print(
                f"    Concentration: {result.concentration_ug_g:.2f} ± {result.concentration_unc:.2f} μg/g"
            )
    ...
    for element, result in results.items():
        ...
        print(
            f"  {element:<8} {conc_str:<20} {result.detection_limit_ug_g:<15.2f} {result.k0_used:<10.4f}"
        )
...
def show_k0_database():
    ...
    for isotope in sorted(K0_DATABASE.keys()):
        ...
        print(
            f"  {isotope:<12} {data.element:<8} {data.k0_Au:<10.4f} {data.Q0:<10.2f} "
...
def main():
    ...
    print("  OK k0-standardization for absolute NAA")
    print("  OK SDC correction factors (saturation, decay, counting)")
    print("  OK Q0(α) epithermal corrections")
    print("  OK Flux wire integration for spectrum characterization")
    print("  OK Nuclear data database with 12+ nuclides")
    print("\nNext steps:")
