#!/usr/bin/env python3
"""Run a full Phase 6 worked example on the RAFM LDRD-backed irradiation corpus.

This script builds an activity-review payload from an existing RAFM analysis JSON and
then runs the Phase 6 CLI workflows:

- inventory-review
- masking-review
- optimization-sweep (DI-FOM, FIM-D, MWDCS, BASS-D, STBD-MR)
...
def _write_summary(
    ...
    for objective, path in objective_outputs.items():
        ...
        if not ranked:
            continue
        top = ranked[0]
        score = top.get("difom_score", top.get("objective_score", top.get("total_score", top.get("total_utility"))))
...
def run_worked_example(sample_id: str, output_root: Path) -> Path:
    output_root.mkdir(parents=True, exist_ok=True)

    activity_review_payload = _build_activity_review_payload(sample_id)
    activity_review_path = output_root / "activity_review.json"
    ...
    inventory_output = output_root / "inventory_review.json"
    _run_cli(
        [
            "inventory-review",
    ...
    masking_output = output_root / "masking_review.json"
    _run_cli(
        [
            "masking-review",
            "--activity-review-file",
    ...
    objective_outputs = {
        "di-fom": output_root / "optimization_di_fom.json",
        "fim-d": output_root / "optimization_fim_d.json",
        "mwdcs": output_root / "optimization_mwdcs.json",
        "bass-d": output_root / "optimization_bass_d.json",
        "stbd-mr": output_root / "optimization_stbd_mr.json",
    }
    ...
    second_schedule_path = output_root / "second_irradiation_schedule.json"
    second_candidates_path = output_root / "second_irradiation_candidates.json"
    ...
    second_output = output_root / "second_irradiation_plan.json"
    _run_cli(
        [
            "second-irradiation-plan",
            "--inventory-file",
    ...
    ffexp_output = output_root / "benchmark_experimental_bundle.ffexp"
    _run_cli(
        [
            "ffexp-export",
            "--activity-review-file",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=(EXAMPLE_ROOT / "results" / "phase6_ldrd_worked_example" / DEFAULT_SAMPLE_ID),
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary_path = run_worked_example(args.sample_id, args.output_root)
