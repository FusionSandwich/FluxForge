...
def compare_astm_and_qg(max_spectra: int = None):
    ...
    print(
        f"Comparing QG activity reporting vs strict ASTM E262 formulas for {len(txt_files)} samples."
    )
    print("=" * 80)
    ...
    for txt_path in txt_files:
        ...
        for i, p in enumerate(qg_peaks):
            ...
            # Format isotope name for k0 lookups (e.g. Sc46 -> Sc-46)
            iso_raw = p.get("isotope", "unk")
            iso_fmt = (
                iso_raw[:-2] + "-" + iso_raw[-2:]
                if len(iso_raw) > 2 and iso_raw[-2:].isdigit()
                else iso_raw
            )

            # Fetch verified values
            k0_rec = get_k0_library_record(iso_fmt)
            cross_sec = float(k0_rec.sigma_0_barn) if k0_rec else 5.0
            if cross_sec <= 0.0:
                cross_sec = 5.0  # fallback if 0
                ...
                "measurements": [
                    {
                        ...
                        "sigma_0_barn": cross_sec,
                        "westcott_g_factor": 1.0,
                        "thermal_self_shielding_factor": 1.0,
                        "f1_epithermal_correction": 1.0,
                    }
                ],
            }
            if hl_s > 1.0 and eff > 0 and p.get("net_counts", 0) > 0:
                astm_res = analyze_astm_e262_plan(plan)
