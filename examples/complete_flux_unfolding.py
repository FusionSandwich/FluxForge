...
def load_all_flux_wires(
    proc_dir: Path, raw_dir: Optional[Path] = None
) -> Dict[str, dict]:
    """
    ...
    if raw_dir and raw_dir.exists():
        for filepath in sorted(raw_dir.glob("*.ASC")):
            try:
                ...
                matched = False
                for sid in samples:
                    if sid.replace(" ", "").replace("@", "_") in sample_id.replace(
                        " ", ""
                    ).replace("@", "_") or sample_id.replace(" ", "").replace(
                        "@", "_"
                    ) in sid.replace(
                        " ", ""
                    ).replace(
...
def plot_spectrum_comparison(
    ...
    ax1.step(
        discrete_result.energy_bounds_eV[:-1],
        discrete_result.flux,
        where="post",
        label="Discrete 10-bin",
    ...
    # GLS spectrum
    ax1.step(
        gls_result.energy_bounds_eV[:-1],
        gls_result.flux,
        where="post",
        label=f"GLS Continuous (χ²={gls_result.chi2:.1f})",
        color=gls_color,
        linewidth=2,
        alpha=0.8,
    )
    ...
    if any(thermal_mask):
        ax2.scatter(
            np.array(reaction_energies)[thermal_mask],
    ...
    if any(fast_mask):
        ax2.scatter(
            np.array(reaction_energies)[fast_mask],
    ...
    # Add labels
    for i, (e, f, lbl) in enumerate(
        zip(reaction_energies, reaction_fluxes, reaction_labels)
    ):
        ax2.annotate(
            lbl,
            (e, f),
    ...
    disc_centers = np.sqrt(
        discrete_result.energy_bounds_eV[:-1] * discrete_result.energy_bounds_eV[1:]
    )
    disc_widths = (
        discrete_result.energy_bounds_eV[1:] - discrete_result.energy_bounds_eV[:-1]
    )
    ...
    gls_centers = np.sqrt(
        gls_result.energy_bounds_eV[:-1] * gls_result.energy_bounds_eV[1:]
    )
    ...
    if model_spectrum is not None:
        ...
        model_centers = (
            model_spectrum["energy_eV"][:-1] + model_spectrum["energy_eV"][1:]
        ) / 2
    ...
    table_data = [
        ...
        [
            "Thermal (<0.5 eV)",
            f"{discrete_result.flux[0]:.2e}",
            f"{np.sum(gls_result.flux[:4]):.2e}",
        ],
        [
            "Epithermal (0.5-100 keV)",
            f"{np.sum(discrete_result.flux[1:7]):.2e}",
            f"{np.sum(gls_result.flux[4:14]):.2e}",
        ],
        [
            "Fast (>100 keV)",
            f"{np.sum(discrete_result.flux[7:]):.2e}",
            f"{np.sum(gls_result.flux[14:]):.2e}",
        ],
        [
            "Total",
            f"{np.sum(discrete_result.flux):.2e}",
    ...
    table_data.extend(
        [
            ...
            [
                ...
                str(
                    sum(
                        1 for r in reactions if getattr(r, "flux_type", "") == "thermal"
                    )
                ),
                "",
            ],
            [
                "Fast reactions",
                str(sum(1 for r in reactions if getattr(r, "flux_type", "") == "fast")),
                "",
            ],
        ]
    )
...
def save_results_to_csv(
    ...
    with open(output_dir / "discrete_spectrum.csv", "w") as f:
        ...
        for i in range(len(discrete_result.flux)):
            f.write(
                f"{discrete_result.energy_bounds_eV[i]:.6e},"
    ...
    with open(output_dir / "gls_spectrum.csv", "w") as f:
        ...
        for i in range(len(gls_result.flux)):
            f.write(
                f"{gls_result.energy_bounds_eV[i]:.6e},"
    ...
    with open(output_dir / "reaction_data.csv", "w") as f:
        ...
        f.write(
            "Sample,Isotope,Reaction,Activity_Bq,Flux_n_cm2_s,Flux_type,E_char_eV\n"
        )
        for rxn in reactions:
            ...
            f.write(
                f"{rxn.sample_id},{rxn.isotope},{rxn.reaction_id},"
...
def main():
    ...
    # === Setup Paths ===
    base_dir = Path("/home/smandych/projects/rafm_irradiation_ldrd")
    proc_dir = base_dir / "irradiation_QG_processed/flux_wires"
    raw_dir = base_dir / "raw_gamma_spec/flux_wires"
    output_dir = Path(
        "/home/smandych/projects/FluxForge/flux_unfolding_results"
    )
    output_dir.mkdir(exist_ok=True)

    # Optional model spectrum
    model_file = Path(
        "/home/smandych/projects/spectra_files/whale_spectrum.txt"
    )
    ...
    for sample_id, data_dict in samples.items():
        ...
        print(
            f"  {sample_id:<30} Proc:{proc_status} Raw:{raw_status} Element:{element:<3} Isotopes:{isotopes}"
        )
    ...
    for i in range(len(discrete_result.flux)):
        print(
            f"  {i+1:<4} {discrete_result.energy_bounds_eV[i]:<12.2e} "
    ...
    print(
        f"  Thermal reactions: {sum(1 for r in proc_reactions if getattr(r, 'flux_type', '') == 'thermal')}"
    )
    print(
        f"  Fast reactions: {sum(1 for r in proc_reactions if getattr(r, 'flux_type', '') == 'fast')}"
    )
    ...
    if thermal_fluxes:
        print(
            f"  Thermal (n,γ): {np.mean(thermal_fluxes):.2e} ± {np.std(thermal_fluxes):.2e} n/cm²/s"
        )
    if fast_fluxes:
        print(
            f"  Fast (n,p):    {np.mean(fast_fluxes):.2e} ± {np.std(fast_fluxes):.2e} n/cm²/s"
        )
