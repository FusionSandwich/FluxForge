...
def extract_cd_ratio_pairs(df: pd.DataFrame) -> Dict[str, CdRatioMeasurement]:
    ...
    for element, data in monitor_data.items():
        ...
        cd_mask = df["Sample"].str.upper().str.contains(
            f"{element.upper()}-CD", regex=False
...
def analyze_uwnr_sample(
    ...
    for meas in sample_measurements:
        print(
            f"  {meas.product_isotope:<12} {meas.net_peak_area:<15,.0f} {meas.efficiency:<12.4f}"
        )
    ...
    for element in sorted(results.keys()):
        ...
        print(
            f"  {element:<8} {result.concentration_ug_g:<15.2f} "
...
def plot_flux_spectrum_from_wires(results_path: Path, output_path: Path):
    ...
    ax1 = axes[0, 0]
    ax1.bar(
        range(len(discrete_df)),
        discrete_df["Flux_n_cm2_s"],
        color="steelblue",
        alpha=0.7,
        edgecolor="navy",
    )
