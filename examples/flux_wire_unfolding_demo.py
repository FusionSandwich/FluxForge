...
def demonstrate_sample_isotope_mapping():
    ...
    for element, isotopes in ELEMENT_TO_ISOTOPES.items():
        ...
        print(
            f"{element:<10} {', '.join(isotopes):<30} {reactions[0] if reactions else ''}"
        )
    ...
    for filepath in files[:6]:  # Show first 6
        try:
            ...
            for nuclide in data.nuclides:
                print(
                    f"{data.sample_id:<30} {nuclide.isotope:<10} "
...
def demonstrate_reaction_rate_extraction(proc_dir: Path):
    ...
    print(
        f"{'Sample':<25} {'Reaction':<20} {'Activity (Bq)':<15} {'Flux (n/cm²/s)':<15} {'Type'}"
    )
    print("-" * 95)
    ...
    for filepath in files:
        try:
            ...
            for rxn in reactions:
                ...
                print(
                    f"{rxn.sample_id[:24]:<25} {rxn.reaction_id[:19]:<20} "
...
def demonstrate_discrete_unfolding(reactions: list):
    ...
    print(
        f"{'Bin':<4} {'E_low (eV)':<12} {'E_high (eV)':<12} {'Flux':<15} {'Reactions'}"
    )
    print("-" * 60)
...
def plot_unfolded_spectrum(
    discrete_result, gls_result, output_file: Path, model_spectrum=None
):
    ...
    ax2.step(
        ...
        gls_result.flux,
        where="post",
        label=f"GLS Adjusted (χ²={gls_result.chi2:.2f})",
        color="green",
        linewidth=2,
    )
    ax2.fill_between(
    ...
    if proc_dir.exists():
        ...
        if reactions:
            ...
            save_spectrum_to_csv(
                discrete_result, gls_result, output_dir / "unfolded_spectrum.csv"
            )
