...
def test_processed_file_parsing():
    ...
    processed_dir = Path(
        "/filespace/s/smandych/CAE/projects/ALARA/rafm_irradiation_ldrd/irradiation_QG_processed/flux_wires"
    )
    ...
    for txt_file in sorted(processed_dir.glob("*.txt")):
        try:
            ...
            if data.efficiency:
                print(
                    f"  Efficiency (C1-C4): [{data.efficiency.C1:.3e}, {data.efficiency.C2:.3e}, "
            ...
            for nuc in data.nuclides:
                print(
                    f"    {nuc.isotope}: {nuc.activity:.4e} ± {nuc.activity_unc:.2e} {nuc.activity_unit} "
...
def test_raw_file_parsing():
    ...
    raw_dir = Path(
        "/filespace/s/smandych/CAE/projects/ALARA/rafm_irradiation_ldrd/raw_gamma_spec/flux_wires"
    )
        ...
        try:
            ...
            if data.has_spectrum:
                ...
                if spec.energies is not None:
                    print(
                        f"  Energy range: {spec.energies[0]:.1f} - {spec.energies[-1]:.1f} keV"
                    )
...
def test_efficiency_calculation():
    ...
    processed_file = Path(
        "/filespace/s/smandych/CAE/projects/ALARA/rafm_irradiation_ldrd/"
...
def test_peak_finding():
    ...
    print(
        f"{'Channel':>10s} {'Energy (keV)':>12s} {'Counts':>12s} {'Significance':>12s}"
    )
    print("-" * 50)
    ...
    for peak in sorted(peaks, key=lambda x: x.index):
        ...
        if significance > 3.0:
            ...
            peak_results.append(
                {"channel": ch, "energy": energy, "significance": significance}
            )
...
def test_activity_calculation():
    ...
    print(
        f"{'Energy':>10s} {'Net Counts':>12s} {'Efficiency':>12s} {'Isotope':>10s} {'Activity (uCi)':>15s}"
    )
    print("-" * 70)
    ...
    for peak in peaks:
        ...
        print(
            f"{peak.energy_keV:10.1f} {peak.net_counts:12.0f} {peak.efficiency:12.4e} "
    ...
    for stem in sorted(matches):
        ...
        if processed_data.energy_calibration:
            ...
            if raw_data.has_spectrum:
                raw_data.spectrum.calibration = {
                    "energy": processed_data.energy_calibration
                }
                raw_data.spectrum.energies = raw_data.channel_to_energy(
                    raw_data.spectrum.channels
                )
        ...
        if raw_data.has_spectrum and raw_data.efficiency:
            ...
            for nuc in processed_data.nuclides:
                if nuc.isotope in activities:
                    ...
                    ratio = (
                        calc["activity_bq"] / nuc.activity_bq
                        if nuc.activity_bq > 0
                        else 0
                    )
                    summary.append(
                        {
