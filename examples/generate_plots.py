...
def load_example_data() -> Dict:
    ...
    return {
        ...
        "number_densities": json.loads(
            (example_dir / "number_densities.json").read_text()
        ),
...
def run_unfolding(data: Dict) -> Tuple[Dict, Dict, Dict, List, List]:
    ...
    gls_result = gls_adjust(
        response.matrix, measured_rates, measurement_cov, prior_flux, prior_cov
    )
    gravel_result = gravel(
        response.matrix, measured_rates, initial_flux=prior_flux, max_iters=500
    )
    mlem_result = mlem(
        response.matrix, measured_rates, initial_flux=prior_flux, max_iters=200
    )
    gd_result = gradient_descent(
        response.matrix, measured_rates, initial_flux=prior_flux, max_iters=5000
    )

    return (
        {
            "flux": gls_result.flux,
            "covariance": gls_result.covariance,
            "chi2": gls_result.chi2,
            "residuals": gls_result.residuals,
        },
...
def plot_spectrum_uncertainty_bands(boundaries, prior, gls, output_dir):
    ...
    ax.set_title(
        "Neutron Flux Spectrum: Prior vs GLS Adjusted\n(Fe-Cd-RAFM-1 Experimental Data)"
    )
    ax.set_xticks(x)
    ax.set_xticklabels(
        [f"{midpoints[g]/1e3:.1f} keV" for g in range(n_groups)], rotation=45
    )
    ax.legend(loc="best")
    ...
    for g in range(n_groups):
        ax.annotate(
            f"{boundaries[g]:.1f}-{boundaries[g+1]:.0f} eV",
...
def plot_prior_posterior_overlay(boundaries, prior, gls, gravel, mlem, output_dir):
    ...
    n_groups = len(prior)
    midpoints = np.array(
        [(boundaries[g] + boundaries[g + 1]) / 2 for g in range(n_groups)]
    )
    ...
    ax.set_ylabel("Flux (arbitrary units)")
    ax.set_title(
        "Prior vs Adjusted Spectrum (Log-Log Scale)\n(Fe-Cd-RAFM-1 Experimental Data)"
    )
    ax.legend(loc="best")
...
def plot_residuals(gls, reaction_ids, output_dir):
    ...
    colors = [
        "green" if abs(r) < 2 else "orange" if abs(r) < 3 else "red" for r in residuals
    ]
...
def plot_parity(measured_rates, gls, response_matrix, output_dir):
    ...
    ax.set_title(
        "Predicted vs Measured Reaction Rates\n(Fe-Cd-RAFM-1 Experimental Data)"
    )
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.legend(loc="best")
...
def plot_solver_comparison(boundaries, prior, gls, gravel, mlem, gd, output_dir):
    ...
    ax1.bar(
        x - 2 * width, prior, width, label="Prior", color=COLORS["prior"], alpha=0.7
    )
    ...
    ax1.bar(
        x + width, mlem["flux"], width, label="MLEM", color=COLORS["mlem"], alpha=0.7
    )
    ax1.bar(
        x + 2 * width, gd["flux"], width, label="GD", color=COLORS["model"], alpha=0.7
    )
    ...
    gravel_change = [
        (f - p) / p * 100 if p > 0 else 0 for f, p in zip(gravel["flux"], prior)
    ]
    mlem_change = [
        (f - p) / p * 100 if p > 0 else 0 for f, p in zip(mlem["flux"], prior)
    ]
...
def plot_response_matrix(response_matrix, boundaries, reaction_ids, output_dir):
    ...
    # Labels
    group_labels = [
        f"{boundaries[g]:.1f}-{boundaries[g+1]:.0f} eV" for g in range(n_groups)
    ]
...
def main():
    ...
    gls, gravel_res, mlem_res, gd_res, measured_rates, response_matrix = run_unfolding(
        data
    )
    ...
    plot_prior_posterior_overlay(
        boundaries, prior, gls, gravel_res, mlem_res, output_dir
    )
    ...
    plot_solver_comparison(
        boundaries, prior, gls, gravel_res, mlem_res, gd_res, output_dir
    )
