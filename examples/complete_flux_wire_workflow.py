...
from fluxforge.physics.activation import calculate_activity, calculate_reaction_rate
...
OUTPUT_DIR = (
    EXAMPLE_DIR.parent.parent.parent.parent / "examples_outputs" / "flux_wire_analysis"
)
...
peaks_found = auto_find_peaks(
    spec.channels, spec.counts, threshold=5.0, min_distance=10, finder_method="scipy"
)
...
peaks_data = {
    ...
    "peaks": [
        {"channel": int(ch), "significance": float(sig)} for ch, sig in peaks_found
    ],
...
if co60_1173_ch and co60_1332_ch:
    ...
    print(
        f"  1173 keV: Centroid={p1173.centroid:.2f} ch, Area={p1173.area:.0f} counts, FWHM={p1173.fwhm:.2f} ch"
    )
    print(
        f"  1332 keV: Centroid={p1332.centroid:.2f} ch, Area={p1332.area:.0f} counts, FWHM={p1332.fwhm:.2f} ch"
    )
    ...
    print(
        f"  Derived Calibration: E = {energy_slope:.4f} * ch + {energy_offset:.2f} keV"
    )
...
print(
    f"  1173 keV correction factor: {corr_1173.factor:.4f} ± {corr_1173.uncertainty:.4f}"
)
print(
    f"  1332 keV correction factor: {corr_1332.factor:.4f} ± {corr_1332.uncertainty:.4f}"
)
...
if co60_1173_ch and co60_1332_ch:
    ...
    print(
        f"  Corrected areas: 1173={corrected_area_1173:.0f}, 1332={corrected_area_1332:.0f}"
    )
...
R = np.array([[1.0, 0.1, 0.01], [0.1, 1.0, 0.1], [0.01, 0.1, 1.0]]) * 1e-25  # barns
...
gls_result = gls_unfold(
    reaction_rates=reaction_rates, response_matrix=R, prior_flux=prior_flux, sigma=sigma
)
...
unfolding_results = {
    ...
    "provenance": generate_provenance(
        "spectrum_unfolding", {"methods": ["GLS", "GRAVEL", "MLEM"]}
    ),
}
