...
def generate_hfir_spectrum(n_groups: int = 100) -> Tuple[np.ndarray, np.ndarray]:
    ...
    thermal = (
        thermal / np.max(thermal) * HFIR_IRRADIATION_PARAMS["thermal_flux_n_cm2_s"]
    )
    ...
    epithermal = (
        epithermal
        / np.max(epithermal + 1e-30)
        * HFIR_IRRADIATION_PARAMS["epithermal_flux_n_cm2_s"]
    )
...
def step2_3_irradiate_and_measure(
    ...
    print(
        f"  Thermal flux: {HFIR_IRRADIATION_PARAMS['thermal_flux_n_cm2_s']:.2e} n/cm²/s"
    )
    ...
    for element in wire_elements:
        ...
        for rxn in reactions:
            ...
            rate, rate_unc = simulate_reaction_rate(
                reaction_str, spectrum_edges_MeV, spectrum_flux, irrad_time, irdff_db
            )
            ...
            if activity_meas > 0:
                ...
                print(
                    f"{reaction_str:<25} {rate:<12.3e} {activity_meas:<15.3e} {rel_unc:<8.1f}"
                )
...
def step4_load_apriori_spectrum(
    ...
    print(
        f"  Energy range: {true_spectrum_edges[0]:.2e} - {true_spectrum_edges[-1]:.2e} MeV"
    )
...
def step5_unfold_spectrum(
    ...
    flux_per_eV = (
        apriori_flux / energy_widths_eV * 1e6
    )  # Convert from per MeV to per eV
...
def step6_compare_spectra(
    ...
    energy_widths_eV = (
        unfolded_result.energy_edges[1:] - unfolded_result.energy_edges[:-1]
    )
...
def step7_calculate_fluences(
    ...
    thermal_fluence = np.sum(
        fluence_per_MeV[thermal_mask] * energy_widths_MeV[thermal_mask]
    )
    epithermal_fluence = np.sum(
        fluence_per_MeV[epithermal_mask] * energy_widths_MeV[epithermal_mask]
    )
    fast_fluence = np.sum(fluence_per_MeV[fast_mask] * energy_widths_MeV[fast_mask])
    fast_1mev_fluence = np.sum(
        fluence_per_MeV[fast_1mev_mask] * energy_widths_MeV[fast_1mev_mask]
    )
...
def run_reactor_dosimetry_workflow(
    ...
    measurements = step2_3_irradiate_and_measure(selected_wires, true_edges, true_flux)
    ...
    print(f"\nSpectrum Quality:")
    print(
        f"  RMS improvement over a priori: {comparison['apriori_rms']/comparison['unfolded_rms']:.2f}x"
    )
    print(f"\nKey Results:")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--groups", type=int, default=100, help="Number of energy groups (100 or 640)"
    )
    parser.add_argument(
        "--method",
        type=str,
