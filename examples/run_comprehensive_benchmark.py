...
def test_gravel_mlem_benchmark():
    ...
    try:
        ...
        gravel_result = gravel(
            R, measurements, initial_flux=x0, max_iters=500, tolerance=1e-6
        )
        ...
        print(f"\nResults:")
        print(
            f"  GRAVEL: {gravel_result.iterations} iterations, converged={gravel_result.converged}"
        )
        print(
            f"  MLEM:   {mlem_result.iterations} iterations, converged={mlem_result.converged}"
        )
        ...
        print(
            f"  GRAVEL flux: min={gravel_flux.min():.2e}, max={gravel_flux.max():.2e}, sum={gravel_flux.sum():.2e}"
        )
        print(
            f"  MLEM flux:   min={mlem_flux.min():.2e}, max={mlem_flux.max():.2e}, sum={mlem_flux.sum():.2e}"
        )
...
def test_dose_calculations():
    ...
    fluence = fluence_from_activity(
        activity, sigma, molar_mass, sample_mass, irrad_time, half_life
    )
...
def test_peak_finders():
    ...
    for i, p in enumerate(refined):
        if p.centroid is not None:
            ...
            error = min(dists)
            print(
                f"    Peak {i+1}: Found={p.centroid:.2f}, Nearest true={true_pos}, Error={error:.2f} ch"
            )
...
def test_regularized_unfolding():
    ...
    print(
        f"  True spectrum: max={true_spectrum.max():.1f}, sum={true_spectrum.sum():.1f}"
    )
    ...
    tik_error = (
        np.sqrt(np.mean((x_tik - true_spectrum) ** 2)) / np.mean(true_spectrum) * 100
    )
    ...
    lcurve_error = (
        np.sqrt(np.mean((x_lcurve - true_spectrum) ** 2)) / np.mean(true_spectrum) * 100
    )
    ...
    grad_error = (
        np.sqrt(np.mean((result.spectrum - true_spectrum) ** 2))
        / np.mean(true_spectrum)
        * 100
    )
