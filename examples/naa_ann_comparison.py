...
RESULTS_CSV = (
    NAA_ANN_1_DIR
    / "ANN code"
    / "code and results versions"
    / "2022-05-05 4e"
    / "NAA1 2022-05-09 4e results.csv"
)
...
def prepare_fluxforge_dataset(
    ...
    n_samples = (
        ...
        else min(max_samples, len(composition_df))
    )
...
def compare_predictions(
    ...
    if y_naaann1 is not None:
        ...
        metrics["naaann1"] = {
            ...
            "R2": 1
            - np.sum(naaann1_error**2) / np.sum((y_real - np.mean(y_real)) ** 2),
        }
...
def main():
    ...
    try:
        ...
        print(f"  ERROR: {e}")
        print(
            "  Run 'unzip Original data/RID data 2022-04-27.zip -d RID_extracted/' first"
        )
        return
    ...
    print("-" * 50)
    print(
        f"{'R²':<20} {metrics['fluxforge']['R2']:.4f}{'':<10} {naaann1_metrics['fluxforge']['R2']:.4f}"
    )
    print(
        f"{'RMSE (ppm)':<20} {metrics['fluxforge']['RMSE']:.4f}{'':<10} {naaann1_metrics['fluxforge']['RMSE']:.4f}"
    )
    print(
        f"{'Mean Ratio':<20} {metrics['fluxforge']['mean_ratio']:.4f}{'':<10} {naaann1_metrics['fluxforge']['mean_ratio']:.4f}"
    )
