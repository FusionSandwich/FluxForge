...
from fluxforge.solvers.iterative import (
    gravel as fluxforge_gravel,
    mlem as fluxforge_mlem,
)
...
def run_benchmark():
    ...
    ref_g_result, ref_g_error = ref_gravel(
        R_filtered, meas_filtered, x0.copy(), tolerance
    )
    ref_g_result = np.array(ref_g_result)

    print("\n--- Running Reference MLEM ---")
    ref_m_result, ref_m_error = ref_mlem(
        R_filtered, meas_filtered, x0.copy(), tolerance
    )
    ...
    ff_g_result, ff_g_error = gravel_reference_style(
        R_filtered, meas_filtered, x0.copy(), max_iters=100, tolerance=tolerance
    )
    ff_g_result = np.array(ff_g_result)
    print(f"FluxForge GRAVEL: {len(ff_g_error)} iterations")

    print("\n--- Running FluxForge MLEM (Reference-style) ---")
    ff_m_result, ff_m_error = mlem_reference_style(
        R_filtered, meas_filtered, x0.copy(), max_iters=100, tolerance=tolerance
    )
    ...
    ax1 = axes[0]
    ax1.plot(
        energy_bins, true_norm, "k-", alpha=0.4, label="ToF Ground Truth", linewidth=2
    )
    ax1.plot(
        energy_bins,
        ref_g_norm,
        "b--",
        label=f"Reference GRAVEL (err={err_ref_g:.3f})",
        linewidth=1.5,
    )
    ax1.plot(
        energy_bins,
        ff_g_norm,
        "r:",
        label=f"FluxForge GRAVEL (err={err_ff_g:.3f})",
        linewidth=1.5,
    )
    ...
    ax2.plot(
        ...
        ref_m_norm,
        "b--",
        label=f"Reference MLEM (err={err_ref_m:.3f})",
        linewidth=1.5,
    )
    ax2.plot(
        energy_bins,
        ff_m_norm,
        "r:",
        label=f"FluxForge MLEM (err={err_ff_m:.3f})",
        linewidth=1.5,
    )
    ...
    ax.plot(
        energy_bins,
        ff_g_norm - ref_g_norm,
        "b-",
        label="GRAVEL: FluxForge - Reference",
        alpha=0.7,
    )
    ax.plot(
        energy_bins,
        ff_m_norm - ref_m_norm,
        "r-",
        label="MLEM: FluxForge - Reference",
        alpha=0.7,
    )
