...
def calculate_reaction_rates(measurements: Dict) -> tuple:
    ...
    for reaction in measurements["reactions"]:
        ...
        print(
            f"  Reaction Rate: {rate_estimate.rate:.3e} ± {rate_estimate.uncertainty:.3e} reactions/s"
        )
        print(
            f"  Relative Unc:  {100 * rate_estimate.uncertainty / rate_estimate.rate:.2f}%"
        )
...
def build_response(
    boundaries: List[float], cross_sections: Dict, number_densities: Dict
) -> tuple:
    ...
    print(
        f"{'Reaction':<20} "
        + " ".join(f"Group{g:>2}" for g in range(groups.group_count))
    )
    for i, rx in enumerate(reactions):
        values = " ".join(
            f"{response.matrix[i][g]:>7.2e}" for g in range(groups.group_count)
        )
...
def unfold_spectrum_gls(
    ...
    print(
        f"Measurement uncertainties: {[f'{100*u/r:.1f}%' for u, r in zip(rate_uncertainties, measured_rates)]}"
    )

    solution = gls_adjust(
        response_matrix, measured_rates, measurement_cov, prior_flux, prior_cov
    )
    ...
    for g in range(len(solution.flux)):
        ...
        print(
            f"{g:<8} {prior:<12.3e} {posterior:<12.3e} {unc:<12.3e} {rel_unc:<12.2f}%"
        )
...
def compare_solutions(
    boundaries: List[float], prior: List[float], gls: Dict, gravel: Dict, mlem: Dict
):
    ...
    for g in range(len(prior)):
        ...
        print(
            f"{g:<8} {e_low:<15.3e} {e_high:<15.3e} {prior[g]:<12.3e} "
    ...
    print(
        f"{'TOTAL':<8} {'---':<15} {'---':<15} {total_prior:<12.3e} "
...
def save_outputs(
    boundaries: List[float], reactions: List, gls: Dict, gravel: Dict, mlem: Dict
):
...
def main():
    ...
    measured_rates, rate_uncertainties, reaction_ids = calculate_reaction_rates(
        data["measurements"]
    )
