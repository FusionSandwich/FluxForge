...
def example_reaction_identifier():
    ...
    for rxn in reactions:
        print(
            f"{rxn.target:<10} {rxn.mt:<6} {rxn.product:<10} {rxn.reaction_string:<25}"
        )
...
def example_endf_irdff_bridge():
    ...
    for mapping in bridge.mappings:
        print(
            f"{mapping.endf_target:<12} {mapping.endf_mt:<6} {mapping.irdff_reaction:<25} {mapping.notes:<30}"
        )
...
def example_temperature_interpolation():
    ...
    print(
        f"  σ(1 eV) at 450 K:  {interp_data.interpolate(1.0):.4f} barns (interpolated)"
    )
