...
# Define paths
DATA_DIR = Path(
    "/filespace/s/smandych/CAE/projects/ALARA/testing/gamma_spec_analysis/test_data"
)
...
def verify_co60():
    ...
    print(
        f"Fit 1173: Centroid={p1.centroid:.2f}, FWHM={p1.fwhm:.2f}, Area={p1.area:.0f}"
    )
    print(
        f"Fit 1332: Centroid={p2.centroid:.2f}, FWHM={p2.fwhm:.2f}, Area={p2.area:.0f}"
    )
    ...
    print(
        f"Correction for 1173 keV: {corr1.factor:.4f} (Uncertainty: {corr1.uncertainty:.4f})"
    )

    # Correct 1332
    corr2 = corrector.calculate_correction("Co60", 1332.5)
    print(
        f"Correction for 1332 keV: {corr2.factor:.4f} (Uncertainty: {corr2.uncertainty:.4f})"
    )
...
def verify_y88():
    ...
    if len(peaks) > 0:
        ...
        p1836 = fit.peak
        print(
            f"Fit 1836 (approx): Centroid={p1836.centroid:.2f}, Area={p1836.area:.0f}"
        )
