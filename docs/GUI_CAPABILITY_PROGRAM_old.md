# FluxForge GUI Capability Program (Archive Copy)

**Status:** Active  
**Scope:** Native Windows/Linux desktop GUI parity, plot parity, offline delivery, and interactive-analysis coverage.

**Roadmap Note (2026-03-30):** The active GUI roadmap now treats `src/fluxforge/gui/`
as the primary Qt redesign surface and keeps `src/fluxforge_gui/` as a legacy/archive
fallback. This capability program still applies to both surfaces where parity is
required, but the new architecture direction is controlled by the roadmap docs and
ADRs.
...
- FluxForge remains a native desktop application without a browser dependency.
- The primary redesign surface is the Qt shell in `src/fluxforge/gui/`.
- The legacy Tk shell in `src/fluxforge_gui/` is retained during migration rather than removed.
...
- Any adopted interactive capability must exist in four places:
  ...
  - desktop GUI surface in the active Qt shell and, where still required for migration, the legacy Tk shell,
