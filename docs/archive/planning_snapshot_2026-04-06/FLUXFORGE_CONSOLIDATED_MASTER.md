...
**Roadmap Execution Note:** Stage 0 governance assets are live on GitHub, Phase 1 is
complete under `src/fluxforge/gui/`, `src/fluxforge/io/`, `src/fluxforge/hal/`, and
`src/fluxforge/data/`, and Phase 2 is now complete in the redesigned Qt shell under
`src/fluxforge/gui/` and `src/fluxforge/core/`. The roadmap status tracker remains
`docs/ROADMAP_EXECUTION_STATUS.md`, and the Tk shell under `src/fluxforge_gui/` is
retained as a legacy/archive fallback while Phase 3.18 follows the completed 3.17
calibration/efficiency parity pass.
...
| C3 | Energy/efficiency calibration UI | Partial | The redesigned Qt path now includes the unified Phase 2.1 workspace plus Phase 2.2-2.6 quick-slider anchors, deviation-pair fine tuning, draggable ROI fitting, the registered skew fitter, the registry-driven method selector, 3.17 detector-slot recall, preserved/fine-tuned calibration flows, NASA-style smart seeding, and all four first-class efficiency models with standards-aware source locking; efficiency/source import wizards remain |
