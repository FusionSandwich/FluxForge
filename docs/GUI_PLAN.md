...
**Status:** Active MVP with implemented prototype + forward plan  
...
2. **Tkinter + Matplotlib**
  ...
  - This is the **currently implemented prototype stack** and is working now.
  - Interaction ergonomics are good enough for the MVP, but still weaker than the longer-term PeakEasy/QuantumGold target.
...
- **New package:** `fluxforge_gui/` that calls core APIs or runs CLI commands.
- **Current implementation:** Tkinter/ttk/Matplotlib prototype inside `fluxforge_gui/`.
- **Planned evolution:** optionally migrate the front-end shell to Dear PyGui while preserving the same core APIs and artifact contracts.
